	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				<li class="">			
					<form method="post" action="billsaccount.php?<?php if(isset($_GET['codeAcc'])){ echo 'codeAcc='.$_GET['codeAcc'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
						...
						{
						?>
							<a href="billsaccount.php?english=english<?php if(isset($_GET['codeAcc'])){ echo '&codeAcc='.$_GET['codeAcc'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divAcc'])){ echo '&divAcc='.$_GET['divAcc'];}?>" class="btn"><?php echo getString(30);?></a>
						<?php
						}else{
						?>
							<a href="billsaccount.php?francais=francais<?php if(isset($_GET['codeAcc'])){ echo '&codeAcc='.$_GET['codeAcc'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divAcc'])){ echo '&divAcc='.$_GET['divAcc'];}?>" class="btn"><?php echo getString(29);?></a>
						<?php
						}
						?>
						<br/>						
...
<div style="text-align:center;margin-top:20px;">
	<a href="billsaccount_hosp.php?codeAcc=<?php echo $_SESSION['codeAcc'];?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn-large" name="billListbtn" style="font-size:20px;height:40px; padding:10px 40px;margin-right:10px;">
...
<table>
	<td style="vertical-align: top; width: auto;">
		...
		<b><h3><?php echo getString(164);?></h3></b>
		...
		$(document).ready( function() {
		  ...
		  $('#q').keyup( function(){
			...
			url : 'search_billsaccounts1.php' , // url du fichier de traitement
			...
			if(isset($_GET['divBill']))
			{
				...
				<table class="tablesorter" cellspacing="0"> 
					...
					<thead>
						<tr>
							...
							<th style="width:10%"><?php echo getString(146);?></th>
					...
					{
						$TotalGnlPatientPrice=($ligneBill->totalgnlprice * $ligneBill->billpercent)/100;
						
						$TotalGnlInsurancePrice= $ligneBill->totalgnlprice - $TotalGnlPatientPrice;
					?>
						<tr style="text-align:center;<?php if($ligneBill->status==0){?>background:rgba(255,255,0, 0.3)<?php ;}?>">
							<td><?php echo $ligneBill->datebill;?></td>
							<td>
								<a href="printBillAccount.php?num=<?php echo $ligneBill->numero;?>&numbill=<?php echo $ligneBill->numbill;?>&cashier=<?php echo $ligneBill->codecashier;?>&datefacture=<?php echo $ligneBill->datebill;?>&idbill=<?php echo $ligneBill->id_bill;?>"><?php echo $ligneBill->numbill;?></a>
							</td>
							<?php
							...
							<td><?php echo $ligneBill->nomassurance.' '.$ligneBill->billpercent.'%';?></td>
							<td><?php echo $ligneBill->totalgnlprice;?></td>
							<td><?php echo $TotalGnlPatientPrice;?>	</td>
							<td><?php echo $TotalGnlInsurancePrice;?></td>
							...
							?>
							<td>
								<a href="traitement_billsaccount.php?idbillDesactif=<?php echo $ligneBill->id_bill?>&code=<?php echo $ligneBill->codeaccount;?>&divAcc=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>"><?php echo getString(167);?><input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
							...
							?>
								<td>
									<a href="traitement_billsaccount.php?idbillActif=<?php echo $ligneBill->id_bill?>&code=<?php echo $ligneBill->codeaccount;?>&divAcc=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>"><?php echo getString(168);?><input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
				...
				<a href="billsaccount.php?page=1&iduser=<?php echo $_SESSION['id'];?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="Show Bills" class="btn-large"><?php echo getString(170);?></a>
			<?php
			}
			?>
			...
			<table class="tablesorter" cellspacing="0"> 
				...
				?>
						<tr style="text-align:center;<?php if($ligne->status==0){?>background:rgba(255,255,0, 0.3)<?php ;}?>">
							...
							<td>
								<a href="printBillAccount.php?num=<?php echo $ligne->numero;?>&numbill=<?php echo $ligne->numbill;?>&cashier=<?php echo $ligne->codecashier;?>&datefacture=<?php echo $ligne->datebill;?>&idbill=<?php echo $ligne->id_bill;?>"><?php echo $ligne->numbill;?></a>
							</td>
							<?php
								$resultPatient=$connexion->prepare('SELECT *FROM utilisateurs u, patients p WHERE u.id_u=p.id_u AND p.numero=:operation');
								$resultPatient->execute(array(
							...
							<td><?php echo $ligne->totalgnlprice;?></td>
							<td><?php echo $TotalGnlPatientPrice;?>	</td>
							<td><?php echo $TotalGnlInsurancePrice;?></td>
							...
							?>
							<td>
								<a href="traitement_billsaccount.php?idbillDesactif=<?php echo $ligne->id_bill?>&code=<?php echo $ligne->codeaccount;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>"><?php echo getString(167);?><input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
							...
							?>
								<td>
									<a href="traitement_billsaccount.php?idbillActif=<?php echo $ligne->id_bill?>&code=<?php echo $ligne->codeaccount;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>"><?php echo getString(168);?><input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
								</td>
							<?php
								}
							}
