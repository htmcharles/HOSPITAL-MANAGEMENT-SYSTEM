...
if($connect==true AND ($comptidL!=0 OR $comptidM!=0 OR $comptidS!=0))
{
	if($status==1)
	{
?>
...
?>
	<div style="text-align:center;margin-top:20px;">
		...
		<a href="stockrecording.php?<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn-large" name="savebtn" style="font-size:20px;height:40px; padding:10px 40px;">
			<?php echo 'Stock';?>
		</a>
	...
	?>
		...
		?>
			...
			if($_SESSION['id']!=13919){
				$resultLaboPa=$connexion->prepare('SELECT * FROM med_labo ml, consultations c WHERE ml.numero=:num AND ml.examenfait=1 AND ml.id_consuLabo=c.id_consu AND c.HiddenFile != 1 '.$dailydateperso.' ORDER BY ml.dateconsu DESC');		
			}else{
				$resultLaboPa=$connexion->prepare('SELECT * FROM med_labo ml, consultations c WHERE ml.numero=:num AND ml.examenfait=1 AND ml.id_consuLabo=c.id_consu '.$dailydateperso.' ORDER BY ml.dateconsu DESC');		
			}
 ...
 -->		  <table cellpadding=3 style="margin:auto auto 10px auto; padding: 10px; width:95%;">
                <tr>
                    <td style="font-size:18px; text-align:center; width:33.333%;">
                        ...
                        <!-- <?php if($_SESSION['id']== 13919){ ?>
                        <a href="patients_laboreport.php?report=ok&chooseReport=ok&alllaborepo=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="text-align:center;<?php if(isset($_GET['reporthospCash'])){ echo 'display:none;';}else{ echo 'display:inline;';}?>">
                            <button id="gnlBillReport" class="btn-large">All Labo Reports</button>
                        </a>
                    	<?php }?> -->
 ...
 -->		  <table cellpadding=3 style="margin:auto auto 10px auto; padding: 10px; width:95%;">
                <tr>
                    <td style="font-size:18px; text-align:center; width:33.333%;">
                        ...
                        <a href="patients_laboreport.php?peek=ok&coordi=<?php echo $_SESSION['id'];?>&laboreportHosppeeked=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="text-align:center;<?php if(isset($_GET['reporthospCash'])){ echo 'display:none;';}else{ echo 'display:inline;';}?>">
                            <button id="gnlBillReport" class="btn-large-inversed">Hospital Report</button>
                        </a>
	...
	?>
					...
					if(isset($_POST['dailydatePerso']))
					{
						...
						if($_POST['dailyIdexaPerso']!=0)
						{
							...
							if($ligneExaPresta=$resultExaPresta->fetch())
							{
								...
								$assuCount = $comptAssuConsu->rowCount();
								$condition = '((ml.id_prestationExa='.$ligneExaPresta->id_prestation.' OR ml.autreExamen LIKE \'%'.$nomExa.'%\' OR ml.autreExamen LIKE \'%'.$nameExa.'%\') AND id_assuLab=1) ';
								for($l=1;$l<=$assuCount;$l++)
								{
									$ligneNomAssu = $comptAssuConsu->fetch();
									$presta_assu = 'prestations_'.strtolower($ligneNomAssu->nomassurance);
									
										//echo '<br>SELECT id_prestation FROM '.$presta_assu.' WHERE nompresta LIKE "'.$nomExa.'%" OR namepresta LIKE "'.$nameExa.'%"<br>';
									
									if($l!=1){
										$selectOtherIdPresta = $connexion->query('SELECT id_prestation FROM '.$presta_assu.' WHERE nompresta LIKE "'.$nomExa.'%" OR namepresta LIKE "%'.$nameExa.'%"');
										$selectOtherIdPresta->setFetchMode(PDO::FETCH_OBJ);
										$row = $selectOtherIdPresta->rowCount();
										//echo "row = ".$row."<br>";
										if ($row != 0) {
											while ($ligneselectOtherIdPresta = $selectOtherIdPresta->fetch()) {
											
												$conditionO = 'OR ((ml.id_prestationExa='.$ligneselectOtherIdPresta->id_prestation.' OR ml.autreExamen LIKE \'%'.$nomExa.'%\' OR ml.autreExamen LIKE \'%'.$nameExa.'%\') AND id_assuLab='.$ligneNomAssu->id_assurance.')';

												$condition = $condition ." ".$conditionO;
											}
										}
									}
							...
							//	$dailydateperso = ' AND ml.dateresultats=\''.$_POST['dailydatePerso'].'\' AND (ml.id_prestationExa='.$ligneExaPresta->id_prestation.' OR ml.autreExamen LIKE \''.$nomExa.'\' OR ml.autreExamen LIKE \''.$nameExa.'\')';
								...
								$dailydateperso = ' AND ml.dateresultats=\''.$_POST['dailydatePerso'].'\' AND ('.$condition.')';
							}
					...
					if(isset($_POST['monthlydatePerso']) AND isset($_POST['monthlydatePersoYear']))
					{
						...
						if($_POST['monthlyIdexaPerso']!=0)
						{
							...
							{						
								...
								$dailydateperso = 'AND ml.dateresultats>=\''.$umwaka.'-'.$ukwezi.'-1\' AND ml.dateresultats<=\''.$umwaka.'-'.$ukwezi.'-'.$daysmonth.'\' AND ('.$condition.')';
					...
					if(isset($_POST['annualydatePerso']))
					{
						...
						if($_POST['annualyIdexaPerso']!=0)
						{
							...
							{							
								...
								$dailydateperso = 'AND ml.dateresultats>=\''.$year.'-01-01\' AND ml.dateresultats<=\''.$year.'-12-31\' AND ('.$condition.')';
					...
					if(isset($_POST['customdatedebutPerso']) AND isset($_POST['customdatefinPerso']))
					{
						...
						if($_POST['customIdexaPerso']!=0)
						{
							...
							{						
								...
								$dailydateperso = 'AND ml.dateresultats>=\''.$debut.'\' AND ml.dateresultats<=\''.$fin.'\' AND ('.$condition.')';
			...
			?>
				...
				<?php
				if(isset($_SESSION['codeS'])){
					$resultLaboPa=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.examenfait=1 '.$dailydateperso.' ORDER BY ml.dateresultats');		
					 $resultLaboPa->execute(array(
					'id_uL'=>$_SESSION['id']
					));
				}else{
		...
		?>
			...
			<div id="selectdatePersoMedicReport">
				...
				<form action="patients_laboreport.php?report=ok&dmac=ok&selectGnlLaboReport=ok&<?php if(isset($_GET['alllaborep'])){echo '&alllaborepo=ok';} ?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>&laboreport=ok" method="post" style="margin:auto;padding:5px;width:90%;" onsubmit="return controlFormCustom(this)" >
				...
				<table style="width:100%;">
					<tr>
						<td style="text-align:center; width:33.333%;">
							...
							<a href="dmacreport_patientLabo.php?dailydateperso=<?php echo $dailydateperso;?>&paVisit=<?php echo $paVisit;?>&divPersoMedicReport=ok&laboreport=ok&alllaborepo=ok" style="text-align:center" id="dmacmedicalpersopreview">
					...
					<table class="tablesorter tablesorter3" cellspacing="0" style="background:#fff;width:100%;"> 
						...
						?>						
								...
								<tr>
									...
									<td style="text-align:center;"><?php echo $TotalDayPrice;?>ddd</td>	
				...
				<table style="width:100%;">
					<tr>
						<td style="text-align:center; width:33.333%;">
							...
							<a href="dmacreport_patientLabo.php?dailydateperso=<?php echo $dailydateperso;?>&paVisit=<?php echo $paVisit;?>&divPersoMedicReport=ok&laboreportpeeked=ok" style="text-align:center" id="dmacmedicalpersopreview">
...
<!-- ----------------------------------------------------------------------------------------------------------------------------------------- -->
	<?php 
			if (isset($_GET['laboreportHosppeeked'])) {
	?>

	<table style="margin:auto;">
                <tr>
                    <td style="font-size:18px; width:33.333%;display:inline" id="individualstring">
                        <b><h2><?php if(isset($_GET['reportclinic'])){ echo 'Labo Clinic Report';} if(isset($_GET['laboreportHosppeeked'])){ echo 'Labo Hospital Report';}?></h2></b>
                    </td>
                </tr>
            </table>
			...
			<div id="selectdatePersoMedicReport">
				...
				<form action="patients_laboreport.php?dmac=ok&selectGnlLaboReport=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>&laboreportHosppeeked=ok" method="post" style="margin:auto;padding:5px;width:90%;" onsubmit="return controlFormCustom(this)" >
			...
			if(isset($_GET['dmac']) OR isset($_GET['selectGnlLaboReport']))
			{
				...
				if(isset($_POST['searchmonthlyPerso']))
				{
					if(isset($_POST['monthlydatePerso']) AND isset($_POST['monthlydatePersoYear']))
					{
						...
						$dailydateperso = 'AND ml.dateresultats>=\''.$umwaka.'-'.$ukwezi.'-01\' AND ml.dateresultats<=\''.$umwaka.'-'.$ukwezi.'-'.$daysmonth.'\' ';
				...
				<table style="width:100%;">
					<tr>
						<td style="text-align:center; width:33.333%;">
							...
							<a href="dmacreport_patientLabo.php?dailydateperso=<?php echo $dailydateperso;?>&paVisit=<?php echo $paVisit;?>&divPersoMedicReport=ok&laboreportHosppeeked=ok" style="text-align:center" id="dmacmedicalpersopreview">
