...
if(!isset($_GET['divPa']))
{
	...
	if(!isset($_GET['showresult']))
	{
		...
		?>
					...
					$selectonmedelab->execute(['numero'=>$ligneC->numero]);
		...
		?>
			...
			?>
				...
				$resultatsConsultPatient=$connexion->prepare('SELECT *FROM utilisateurs u, patients p, consultations c WHERE u.id_u=p.id_u AND c.numero=p.numero AND c.id_uM=:idMed AND c.dateconsu=:annee GROUP BY c.numero ORDER BY c.heureconsu DESC');
				...
				$resultatsConsultPatientFinished=$connexion->prepare('SELECT *FROM utilisateurs u, patients p, consultations c WHERE u.id_u=p.id_u AND c.numero=p.numero AND c.id_uM=:idMed AND c.dateconsu=:annee AND c.done = 1 GROUP BY c.numero ORDER BY c.heureconsu DESC');

				$resultatsConsultPatientFinished->execute(array(
				'idMed'=>$_SESSION['id'],
				'annee'=>$annee
				));

				$resultatsConsultPatientFinished->setFetchMode(PDO::FETCH_OBJ);
			
				$comptConsultPatientFinished=$resultatsConsultPatientFinished->rowCount();
