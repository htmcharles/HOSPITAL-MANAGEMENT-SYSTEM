...
try
{
	...
	if(isset($_POST['savebtn']))
	{
		...
		if($_POST['profil']!="Patient")
		{
			$resultats=$connexion->prepare('INSERT INTO utilisateurs (nom_u,prenom_u,full_name,sexe,province,autreadresse,telephone,e_mail,password,datamanager,status,district,secteur,cell,village,updatedby) VALUES(:nom, :prenom, :fullname, :sexe, :province, :adresseExt, :phone, :e_mail, :password,:dataM,:status,:distict,:secteur,:cell,:village,:updatedby)');
		...
		if($_POST['profil']=="Patient")
		{
			...
			{			
				...
				$insertUti=$connexion->prepare('INSERT INTO utilisateurs (nom_u,prenom_u,full_name,sexe,province,autreadresse,telephone,e_mail,password,status,district,secteur,cell,village,updatedby) VALUES(:nom, :prenom, :fullname, :sexe, :province, :adresseExt, :phone, :e_mail, :password,:status,:distict,:secteur,:cell,:village,:updatedby)');
				...
				if($ligne=$res->fetch())
				{
					...
					if(isset($_POST['checkprestaLab'])){
						$consuType = $_POST['consuType'];
						$id_uM = $_POST['id_uM'];
						$prixConsult = 0.1;
						$prestaLab = array();

						foreach($_POST['checkprestaLab'] as $examName){
							$prestaLab[] = $examName;
						}
						...
						if($lignePa=$getSNpatient->fetch())
						{
							...
							if($lastConsu  = $getLastConsu->fetch(PDO::FETCH_OBJ)){
								...
								for($i=0;$i<sizeof($prestaLab);$i++)
								{

									$reqlabo=$connexion->query('SELECT * FROM '.$presta_assu.' WHERE `nompresta` LIKE "%'.$prestaLab[$i].'%"');
									if($lignePri = $reqlabo->fetch(PDO::FETCH_OBJ)){
										$prixLabo = $lignePri->prixpresta;
										$prestaExa = $lignePri->id_prestation;
									}
					...
					if(isset($_SESSION['infhosp']))
					{
						...
						{
							if(isset($_POST['checkprestaLab'])){
								$pahosp="printBill.php?cashier=".$_SESSION['id']."&num=".$lignePa->numero."&idmed=".$id_uM."&dateconsu=".$dateconsu."&idtypeconsu=".$idtypeconsu."&idassu=".$idassu."&idconsu=".$lastId."&idbill=''&createfacture=0";
							}else{
	...
	if(isset($_POST['updatebtn']))
	{
		...
		if(($_POST['village']))
		{
			$newVillage=strip_tags($_POST['village']);
		}else{
			$newVillage="";
		}
		...
		if( $num_rows != 0)
		{
			$resultats=$connexion->prepare('UPDATE utilisateurs SET nom_u=:nom, prenom_u=:prenom, full_name=:fullname, sexe=:sexe, province=:province, autreadresse=:adresseExt, telephone=:phone, e_mail=:mail, password=:password, datamanager=:newDataM, district=:district, secteur=:secteur,cell=:cell,village=:village, updatedby=:idupdatedby WHERE id_u=:iduti');
