...
$(document).ready( function() {
  ...
  $('#s').keyup( function(){
	...
	if(isset($_GET['divRec']))
	{
		...
		if($comptidA!=0)
		{
			$resultatsA=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.id_u=:idRec AND u.id_u=:idRec AND r.codereceptio LIKE \'CNGR%\' ');
		...
		if($comptidM!=0)
		{
			$resultatsM=$connexion->prepare('SELECT *FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.id_u=:idRec AND u.id_u=:idRec AND r.codereceptio LIKE \'CNGR%\' ');
	...
	try
	{
		...
		// Numero de page (1 par défaut)
			...
			/*-----------Requête pour coordinateur-----------*/
		$resultats=$connexion->query('SELECT * FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio LIKE \'CNGR%\' ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
	...
	<tr>
		...
		$nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u, receptionistes r WHERE u.id_u=r.id_u AND r.codereceptio LIKE \'CNGR%\' ORDER BY u.nom_u');
