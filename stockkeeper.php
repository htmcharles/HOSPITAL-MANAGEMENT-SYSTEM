...
<head>
	<title>AFFICHE LABORANTINS</title>
	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				<li class="">			
					<form method="post" action="stockkeeper.php?<?php if(isset($_GET['codeCoord'])){ echo 'codeCoord='.$_GET['codeCoord'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
					...
					{
					?>
						<a href="stockkeeper.php?english=english<?php if(isset($_GET['codeCoord'])){ echo '&codeCoord='.$_GET['codeCoord'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divLabo'])){ echo '&divLabo='.$_GET['divLabo'];}?>" class="btn"><?php echo getString(30);?></a>
					<?php
					}else{
					?>
						<a href="stockkeeper.php?francais=francais<?php if(isset($_GET['codeCoord'])){ echo '&codeCoord='.$_GET['codeCoord'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divLabo'])){ echo '&divLabo='.$_GET['divLabo'];}?>" class="btn"><?php echo getString(29);?></a>
					<?php
					}					
					?>
						<br/>						
...
?>
			<h2><?php echo getString(281);?>s</h2>
...
$(document).ready( function() {
  ...
  $('#q').keyup( function(){
	...
	url : 'traitement_coordinateurs1.php?name=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
...
$(document).ready( function() {
  ...
  $('#s').keyup( function(){
	...
	url : 'traitement_coordinateurs1.php?sn=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
...
<?php 
	if(isset($_GET['divSto']))
	{
		...
		if($comptidS!=0)
		{
			$resultatsS=$connexion->prepare('SELECT *FROM utilisateurs u, stockkeeper sto WHERE u.id_u=sto.id_u AND sto.id_u=:idSto AND u.id_u=:idSto');
			$resultatsS->execute(array(
			'idSto'=>$_GET['iduti']	
			))or die( print_r($connexion->errorInfo()));
		

			$resultatsS->setFetchMode(PDO::FETCH_OBJ);//on veut que le résultat soit récupérable sous forme d'objet
		}
	...
	?>
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				<?php
				while($ligneS=$resultatsS->fetch())//on récupère la liste des éléments
				{
				?>
				<tr style="text-align:center;<?php if($ligneS->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneS->codestock;?></td>
					<td><?php echo $ligneS->nom_u;?></td>
					<td><?php echo $ligneS->prenom_u;?></td>
					<td>
					<?php 
					if($ligneS->sexe=="M")
					{
						echo "Male";
					}elseif($ligneS->sexe=="F"){
						echo "Female";
					}else{
						echo "";
					}
					...
					<?php
						...
						'idProv'=>$ligneS->province,
						'idDist'=>$ligneS->district,
						'idSect'=>$ligneS->secteur
						));
						...
						{
							if($ligneAdresse->id_province == $ligneS->province)
							{
								$adresse = $ligneAdresse->nomprovince.', '.$ligneAdresse->nomdistrict.', '.$ligneAdresse->nomsector;
								
							}
						}elseif($ligneS->autreadresse!=""){
								$adresse=$ligneS->autreadresse;
						}else{
							$adresse="";
					...
					<td><?php echo $ligneS->dateaffectionstock;?></td>
					...
					?>
					<td>
						<a href="utilisateurs.php?iduti=<?php echo $ligneS->id_u?>"><input type="image" src="images/icn_edit.png" title="Modifier"/></a>
					</td>

					<td>
						<a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligneS->id_u?>&code=<?php echo $ligneS->codestock;?>&divSto=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
					...
					?>
						<td></td>
						<td>
							<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligneS->id_u?>&code=<?php echo $ligneS->codestock;?>&divSto=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
		...
		<a href="coordinateurs1.php?iduser=<?php echo $id;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="Show all Managers" class="btn-large">Show all Managers</a>
	<?php
	}
	?>
	...
	try
	{
		...
		$resultats=$connexion->query('SELECT *FROM utilisateurs u, stockkeeper sto WHERE u.id_u=sto.id_u ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
	...
	<tr>
		...
		$nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u, coordinateurs coo WHERE u.id_u=coo.id_u ORDER BY u.nom_u');
