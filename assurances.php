...
<head>
    <meta charset="utf-8"/>
    <title><?php echo 'Assurances';?></title>
...
if($connect==true)
{
    if($status==1)
    {
                        ...
                        <ul class="nav pull-right">
                            <li class="">
                                <form method="post" action="assurances.php?<?php if(isset($_GET['recu'])){ echo '&recu='.$_GET['recu'];} if(isset($_GET['envoye'])){ echo '&envoye='.$_GET['envoye'];} if(isset($_GET['ecrire'])){ echo '&ecrire='.$_GET['ecrire'];} if(isset($_GET['idMsg'])){ echo '&idMsg='.$_GET['idMsg'];} if(isset($_GET['idMsgRecu'])){ echo '&idMsgRecu='.$_GET['idMsgRecu'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
                                    ...
                                    {
                                        ?>
                                        <a href="assurances.php?english=english<?php if(isset($_GET['recu'])){ echo '&recu='.$_GET['recu'];} if(isset($_GET['envoye'])){ echo '&envoye='.$_GET['envoye'];} if(isset($_GET['ecrire'])){ echo '&ecrire='.$_GET['ecrire'];} if(isset($_GET['idMsg'])){ echo '&idMsg='.$_GET['idMsg'];} if(isset($_GET['idMsgRecu'])){ echo '&idMsgRecu='.$_GET['idMsgRecu'];}?>" class="btn"><?php echo getString(30);?></a>
                                        <?php
                                    }else{
                                        ?>
                                        <a href="assurances.php?francais=francais<?php if(isset($_GET['recu'])){ echo '&recu='.$_GET['recu'];} if(isset($_GET['envoye'])){ echo '&envoye='.$_GET['envoye'];} if(isset($_GET['ecrire'])){ echo '&ecrire='.$_GET['ecrire'];} if(isset($_GET['idMsg'])){ echo '&idMsg='.$_GET['idMsg'];} if(isset($_GET['idMsgRecu'])){ echo '&idMsgRecu='.$_GET['idMsgRecu'];}?>" class="btn"><?php echo getString(29);?></a>
                                        <?php
                                    }
                                    ?>
                                    <br/>
        ...
        <?php }?>
                ...
                <div style="display:none;margin-bottom:5px;" id="divListe" align="center">
                    <a href="patients1.php?iduser=<?php echo $_SESSION['id'];?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn"><?php echo getString(19);?></a>
        ...
        <?php
        if(!isset($_GET['editAssu'])) {
            ?>
            <div id="formExam" style="padding:10px 50px; text-align:center;">
                ...
                <form method="post" action="insertionSQL.php" style="margin:auto;">
                    ...
                    */
                    ...
                    ?>
                        ...
                        <input type="text" name="prixCCO[]" id="prixCCO" placeholder="Prix ra" style="margin-top:10px;margin-bottom:0;height:20px;width:100px;"/>
                    ...
                    <table style="background:#ddd;" class="printPreview tablesorter3" cellspacing="0" align="center">
                        ...
                        <tr>
                            ...
                            <th style="border-right: 1px solid #bbb;text-align:center;">Prix de référence</th>
                        </tr>
                        </thead>
                        ...
                        for ($q = 0; $q < 5; $q++) {
                            ...
                            <tr>
                                <td style="text-align:center;">
                                    <input type="text" name="nomAssu[]" id="nomAssu" placeholder="Nom_Assurance"
                                           style="margin:5px;height:20px;width:auto;"/>
                                </td>

                                <td style="text-align:center;">
                                    <p style="margin:auto;">
                                        <select style="margin:auto;" name="prixref[]" class="chosen-select"
                                                id="prixref<?php echo $q ?>">
                                            ...
                                            while ($ligneAssurance = $resultatsAssurance->fetch()) {
                                                ?>
                                                <option value='<?php echo $ligneAssurance->nomassurance; ?>' <?php if ($ligneAssurance->id_assurance == 1) {
                            ...
                            <tr>
                                <td style="background:#aaa;" colspan=2>
                                </td>
                            </tr>
                    ...
                    <input type="submit" name="confirmAssu" id="confirmAssu" class="btn" value="Save"/>

                    <input type="submit" name="confirmExam" id="confirmExam" class="btn" style="display:none;"/>
        ...
        <?php
            ...
            ?>
            <div id="formEditExam" style="display:inline;">
                <?php
                    if(isset($_GET['page']))
                    {
                ...
                ?>
                <form method="post" action="insertionSQL.php?<?php echo $pageTable;?>" style="margin:auto;">

                    <table style="background:#ddd;" class="printPreview tablesorter3" cellspacing="0" align="center">
                        <thead>
                        <tr>
                            <th style="border-right: 1px solid #bbb;text-align:center;">Ancien Assurance</th>
                            <th style="border-right: 1px solid #bbb;text-align:center;">Nouvelle Assurance</th>
                        </tr>
                        </thead>
                        ...
                        <tbody>
                            <tr>
                                <td style="text-align:center;">
                                    <input type="text" name="oldIdAssu" id="oldIdAssu" placeholder="Nom_Assurance" style="margin:5px;height:20px;width:auto;display:none;" value="<?php echo $_GET['editAssu'];?>" readonly="readonly"/>
                                    <input type="text" name="oldNomAssu" id="oldNomAssu" placeholder="Nom_Assurance" style="margin:5px;height:20px;width:auto;" value="<?php echo $_GET['nomAssu'];?>" readonly="readonly"/>
                                </td>

                                <td style="text-align:center;">
                                    <input type="text" name="newNomAssu" id="newNomAssu" placeholder="Tapez_ici.." style="margin:5px;width:80%;"/>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <input type="submit" name="editAssu" id="editAssu" class="btn" value="Save changes"/>
                </form>
            </div>
            <?php
        }
        ?>
            <div style="margin-top:10px;" id="tableAssu">
                <h2><?php echo 'Liste des Assurances';?></h2>
                        ...
                        $('#q').keyup( function(){
                            ...
                            if( $field.val().length > 0 )
                            {
                                ...
                                $.ajax({
                                    ...
                                    url : 'traitement_assurances.php?name=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
                ...
                <?php
                if(isset($_GET['divAssu']))
                {

                    /*-----------Requête pour coordinateur-----------*/

                    $resultatsA=$connexion->prepare('SELECT *FROM assurances WHERE id_assurance=:idassu');
                    ...
                    ?>
                    <table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:30%;margin-bottom:20px;">
                        ...
                        while($ligneA=$resultatsA->fetch())
                        {
                            ...
                            <tr style="text-align:center;">
                                <td><?php echo $ligneA->nomassurance;?></td>
                                <td>
                                    <a href="assurances.php?editAssu=<?php echo $ligneA->id_assurance;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}?>" class="btn"><i class="fa fa-pencil fa-1x fa-fw"></i></a>
                ...
                <table class="tablesorter" cellspacing="0" style="width: 50%">
                    ...
                    try
                    {
                        ...
                        $resultats=$connexion->query('SELECT * FROM assurances ORDER BY nomassurance LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));

                        $resultats->setFetchMode(PDO::FETCH_OBJ);//on veut que le résultat soit récupérable sous forme d'objet
                        //echo 'SELECT * FROM assurances ORDER BY nomassurance LIMIT '.$limit_start.', '.$pagination.'';
                        echo $limit_start.', '.$pagination;

                        while($ligne=$resultats->fetch())//on recupere la liste des éléments
                        {
                            ?>
                            <tr style="text-align:center;">
                                <td><?php echo $ligne->nomassurance;?></td>
                                <td>
                                    <a href="assurances.php?editAssu=<?php echo $ligne->id_assurance;?>&nomAssu=<?php echo $ligne->nomassurance;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}?>" class="btn"><i class="fa fa-pencil fa-1x fa-fw"></i></a>
                ...
                <tr>
                    <td>
                        ...
                        $nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM assurances ORDER BY nomassurance');
