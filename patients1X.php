  ...
  $('#r').keyup( function(){
    ...
    if(isset($_GET['divPa']))
	{
		...
		if($comptidL!=0)
		{
			...
			$resultatsL=$connexion->prepare('SELECT *FROM utilisateurs u, patients p WHERE u.id_u=p.id_u AND u.full_name=:fullname');
...
?>
				<tr style="text-align:center;">
					...
					<td>
					<?php
					$getMedinf=$connexion->prepare('SELECT *FROM utilisateurs u, patients p, med_inf mi WHERE u.id_u=p.id_u AND mi.numero=p.numero AND p.numero=:num AND mi.soinsfait=0 AND u.full_name LIKE \'%'.$_GET['fullname'].'%\' ');
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($comptDoneResultsPa!=0){ echo 'background:rgba(0,100,255,0.5);';}?>">
					...
					{
						/* if($ligneMedLab->id_factureMedLabo!=0)
						{ */
						?>
	...
	if(!isset($_GET['showresult']))
	{
		...
		$annee = date('Y').'-'.date('m').'-'.date('d');
			...
			*/
				...
				if($comptidC!=0)
				{
					$resultatsC=$connexion->query("SELECT *FROM utilisateurs u, patients p, consultations c WHERE u.id_u=p.id_u AND c.numero=p.numero AND c.dateconsu='".$annee."' AND (c.done=0 OR c.done IS NULL) ORDER BY c.id_consu DESC") or die( print_r($connexion->errorInfo()));
		...
		?>
					<tr style="text-align:center;<?php if($lineC->status==0){?>background:rgba(255,0,0,0.15)<?php ;}?><?php if($totalDettes!=NULL){?>background:rgba(255,100,255,0.5)<?php }?>">
						...
						{
						?>
							<a style="padding:10px 20px;" href="<?php echo 'categoriesbill.php?cashier='.$_SESSION['id'].'&num='.$lineC->numero.'&idconsu='.$lineC->id_consu.'&idmed='.$lineC->id_uM.'&dateconsu='.$lineC->dateconsu.'&idtypeconsu='.$lineC->id_typeconsult.'&idassu='.$lineC->id_assurance.'&idbill='.$lineC->id_factureConsult.'&previewprint=ok'?><?php if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}else{ if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}}?><?php if($lineC->id_factureConsult !=NULL){ echo '&newdette=ok';}?>" class="btn-large" name="showpreviewbtn" id="showpreviewbtn"><?php echo getString(220)?></a>
						<?php
						}
		...
		?>
			...
			?>
				...
				$resultatsConsultPatient=$connexion->prepare('SELECT *FROM utilisateurs u, patients p, consultations c WHERE u.id_u=p.id_u AND c.numero=p.numero AND c.id_uM=:idMed AND c.dateconsu=:annee GROUP BY c.numero ORDER BY c.heureconsu');
				...
				$attenteResultsPatient=$connexion->prepare('SELECT *FROM utilisateurs u, patients p, consultations c, med_labo ml WHERE u.id_u=p.id_u AND c.numero=p.numero AND ml.numero=p.numero AND c.id_uM=:idMed AND c.id_consu=ml.id_consuLabo AND c.dateconsu=:annee GROUP BY c.numero ORDER BY c.numero');
				...
				$noResultsPatient=$connexion->prepare('SELECT *FROM utilisateurs u, patients p, consultations c WHERE u.id_u=p.id_u AND c.numero=p.numero AND c.id_uM=:idMed AND c.dateconsu=:annee AND c.done=1 GROUP BY c.numero ORDER BY c.numero');
				...
				<h2 style="font-size:50px;margin:10px;font-weight:100;"><?php echo /* getString(219) */'Liste des patients d\'aujourd\'hui';?></h2>
				...
				<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;">
					...
					{
						$presta_assu='prestations_'.$ligneConsultPatient->assuranceConsuName;
					?>
						<tr style="text-align:center;">
							...
							if($comptMedConsult!=0)
							{
								...
								{
									if($ligneConsult->id_factureConsult!=NULL OR ($nomTypeConsult =="Pas de consultation" OR $nomTypeConsult =="No Consultation"))
									{
							?>
			...
			<?php
				...
				<div style="overflow:auto;max-height:350px;margin:auto;width:80%;display:block">
		...
		if($comptidI!=0)
		{
			...
			if(isset($_GET['soinsPa']))
			{
				$resultatsInfPa=$connexion->prepare("SELECT *FROM utilisateurs u, patients p, consultations c WHERE u.id_u=p.id_u AND c.numero=p.numero AND c.numero=:numPa ORDER BY c.dateconsu DESC");
		...
		?>
						<tr style="text-align:center;">
							...
							{
							?>
								<a href="patients1.php?num=<?php echo $ligneI->numero;?>&soinsPa=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn"><?php echo 'Soins à faire';?></a>
							<?php
							}
							?>
							</td>
