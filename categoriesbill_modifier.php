...
if(isset($_GET['deleteMedConsu']))
{
	...
	echo '<script type="text/javascript">document.location.href="categoriesbill.php?cashier='.$_GET['manager'].'&num='.$_GET['num'].'&idconsu='.$_GET['idconsu'].'&idmed='.$_GET['idmed'].'&dateconsu='.$_GET['dateconsu'].'&idtypeconsu='.$_GET['idtypeconsu'].'&idassu='.$_GET['idassu'].'&idbill='.$_GET['idbill'].'&previewprint=ok";</script>';
...
// 	$updateIdFactureConsult=$connexion->prepare('UPDATE consultations c SET c.id_factureConsult=:idbill, c.codecashier=:codecashier, c.codecoordi=:codecoordi, c.done=1 WHERE c.id_factureConsult=:idbill AND c.id_factureConsult IS NULL AND c.numero=:num');
...
// 	$updateIdFactureMedConsult=$connexion->prepare('UPDATE med_consult mc SET mc.id_factureMedConsu=:idbill, mc.codecashier=:codecashier, mc.codecoordi=:codecoordi WHEREmc.id_factureMedConsu=:idbill AND mc.numero=:num AND mc.id_factureMedConsu=0');
...
// 	$updateIdFactureMedInf=$connexion->prepare('UPDATE med_inf mi SET mi.id_factureMedInf=:idbill, mi.codecashier=:codecashier, mi.codecoordi=:codecoordi WHERE mi.id_factureMedInf=:idbill AND mi.numero=:num AND mi.id_factureMedInf=0');
...
// 	$updateIdFactureMedLabo=$connexion->prepare('UPDATE med_labo ml SET ml.id_factureMedLabo=:idbill, ml.codecashier=:codecashier, ml.codecoordi=:codecoordi WHERE ml.id_consuLabo=:idconsu AND ml.numero=:num AND ml.id_factureMedLabo=0');
...
// 	$updateIdFactureMedLabo=$connexion->prepare('UPDATE med_radio mr SET mr.id_factureMedRadio=:idbill, mr.codecashier=:codecashier, mr.codecoordi=:codecoordi WHERE mr.id_consuRadio=:idconsu AND mr.numero=:num AND mr.id_factureMedRadio=0');
...
// 	$updateIdFactureMedConsom=$connexion->prepare('UPDATE med_consom mco SET mco.id_factureMedConsom=:idbill, mco.codecashier=:codecashier, mco.codecoordi=:codecoordi WHERE mco.id_consuConsom=:idconsu AND mco.numero=:num AND mco.id_factureMedConsom=0');
...
// 	$updateIdFactureMedMedoc=$connexion->prepare('UPDATE med_medoc mdo SET mdo.id_factureMedMedoc=:idbill, mdo.codecashier=:codecashier, mdo.codecoordi=:codecoordi WHERE mdo.id_consuMedoc=:idconsu AND mdo.numero=:num AND mdo.id_factureMedMedoc=0');
...
function controlFormCategoBill(theForm){
	...
	if (rapportPrixTypeConsu != "" || rapportPercentTypeConsu != "" || rapportPrixConsu != "" || rapportPercentConsu != "" || rapportPrixInf != "" || rapportPercentInf != "" || rapportPrixLab != "" || rapportPercentLab != "" || rapportPrixRad != "" || rapportPercentRad != "" || rapportPrixConsom != "" || rapportQuantityConsom != "" || rapportPercentConsom != "" || rapportPrixMedoc != "" || rapportQuantityMedoc != "" || rapportPercentMedoc != "") {
...
<?php
if(isset($_SESSION['codeR']) AND isset($_SESSION['codeC']))
{
...
<?php
	...
	$resultConsult=$connexion->prepare('SELECT *FROM consultations c WHERE c.id_factureConsult=:id_Bill AND c.numero=:num AND c.dateconsu!="0000-00-00" AND c.id_factureConsult IS NOT NULL ORDER BY c.id_consu');		
	...
	$resultMedConsult=$connexion->prepare('SELECT *FROM med_consult mc WHERE mc.numero=:num AND mc.id_factureMedConsu!=0 AND mc.id_factureMedConsu=:id_Bill ORDER BY mc.id_medconsu');		
	...
	$resultMedInf=$connexion->prepare('SELECT *FROM med_inf mi WHERE mi.soinsfait=0 AND mi.numero=:num AND mi.id_factureMedInf!=0 AND mi.id_factureMedInf=:id_Bill ORDER BY mi.id_medinf');		
	...
	$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.examenfait=0 AND ml.numero=:num AND ml.id_factureMedLabo!=0 AND ml.id_factureMedLabo=:id_Bill ORDER BY ml.id_medlabo');		
	...
	$resultMedRadio=$connexion->prepare('SELECT *FROM med_radio mr WHERE mr.radiofait=0 AND mr.numero=:num AND mr.id_factureMedRadio!=0 AND  mr.id_factureMedRadio=:id_Bill ORDER BY mr.id_medradio');		
	...
	$resultMedConsom=$connexion->prepare('SELECT *FROM med_consom mco WHERE mco.numero=:num AND mco.id_factureMedConsom!=0 ANDmco.id_factureMedConsom=:id_Bill ORDER BY mco.id_medconsom');		
	...
	$resultMedMedoc=$connexion->prepare('SELECT *FROM med_medoc mdo WHERE mdo.numero=:num AND mdo.id_factureMedMedoc!=0 AND mdo.id_factureMedMedoc=:id_Bill ORDER BY mdo.id_medmedoc');		
...
?>
	...
	<form method="post" action="addupdatepresta.php?manager=<?php echo $_GET['manager'];?>&numbill=<?php echo $_GET['numbill'];?>&nomassurance=<?php echo $nomassurance;?>&billpercent=<?php echo $_GET['billpercent'];?>	&num=<?php echo $_GET['num'];?>&idbill=<?php echo $_GET['idbill'];?>&finishbtn=ok" enctype="multipart/form-data">
		
		<table align="center">
		   	<?php while($ligneconsultation=$resultConsult->fetch()){ ?>
			<tr>
			...
			<tr>
				...
				<td>
					...
					$resultatsTypeConsu=$connexion->prepare('SELECT *FROM '.$presta_assu.' p WHERE p.id_prestation=:idPresta ORDER BY p.id_prestation');
					$resultatsTypeConsu->execute(array(
						'idPresta'=>$ligneconsultation->id_typeconsult
					));
		...
		<table style="margin:20px auto;">
			...
			if($comptConsult!=0 OR $comptMedConsult!=0 OR $comptMedInf!=0 OR $comptMedLabo!=0 OR $comptMedRadio!=0 OR $comptMedConsom!=0 OR $comptMedMedoc!=0)
			{
			?>
				<td>
				<?php
				...
				{
				?>
					<a style="padding:10px 40px;" href="<?php echo 'formModifierBill.php?manager='.$_GET['manager'].'&num='.$_GET['num'].'&idassu='.$idassu.'&nomassurance='.$nomassurance.'&billpercent='.$_GET['billpercent'].'&idbill='.$_GET['idbill'].'&numbill='.$_GET['numbill'].'&finishbtn=ok'?>" class="btn-large-inversed" name="showpreviewbtn" id="showpreviewbtn"><?php echo getString(220) ?></a>
				<?php
				}/* else{
					if(isset($_GET['previewprint']))
					{
				?>
					<a style="padding:10px 40px;" href="<?php echo 'categoriesbill.php?manager='.$_GET['manager'].'&num='.$_GET['num'].'&idconsu='.$_GET['idconsu'].'&idmed='.$_GET['idmed'].'&dateconsu='.$_GET['dateconsu'].'&idtypeconsu='.$_GET['idtypeconsu'].'&idassu='.$_GET['idassu'].'&idbill='.$_GET['idbill'].'&back=ok'?>" class="btn-large-inversed" name="showbackbtn" id="showbackbtn"><?php echo getString(221) ?></a>
				<?php
					}
				} */
				?>
			...
			<table id="showService" align="center" style="margin:20px auto; display:none;width:100%;background:#ddd;">
				<tr>
					<td style="padding:5px;text-align:center;">
						...
						$resultatsCategoPrestaConsu=$connexion->query('SELECT *FROM categopresta_ins cp WHERE cp.id_categopresta!=1 AND cp.id_categopresta!=2 AND cp.id_categopresta!=3 AND cp.id_categopresta!=12 AND cp.id_categopresta!=13 AND cp.id_categopresta!=14 AND cp.id_categopresta!=21 AND cp.id_categopresta!=22 AND cp.id_categopresta!=23 AND cp.id_categopresta!=4 ORDER BY cp.nomcategopresta');
					...
					<td style="text-align:left;">
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style='text-align:center;background:#ddd;'>	
									<td>Nouveau Prix ra</td>
									<?php
									for($q=0;$q<5;$q++)
									{
									?>	
									<td>
										<input type='text' name='autreprixprestaServCCO[]' id='autreprixprestaServCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>	
								...
								<tr style='text-align:center;background:#ddd;'>
									<td>Nouveau prix <?php echo $nomassurance; ?></td>
									<?php
									for($q=0;$q<5;$q++)
									{
									?>
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaInfCCO[]' id='autreprixprestaInfCCO' style='width:70px;'/>
									</td>
									<?php
									}
									?>	
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaLabCCO[]' id='autreprixprestaLabCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>	
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaRadCCO[]' id='autreprixprestaRadCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaConsomCCO[]' id='autreprixprestaConsomCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>	
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaMedocCCO[]' id='autreprixprestaMedocCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>	
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaOrthoCCO[]' id='autreprixprestaOrthoCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>	
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style="background:#ddd;">	
									...
									?>	
									<td>
										<input type='text' name='autreprixprestaSurgeCCO[]' id='autreprixprestaSurgeCCO' style='width:70px'/>
									</td>
									<?php
									}
									?>	
        ...
        <div id="divViewKine" style="margin:40px auto 0; display:none;">
            ...
            <table id="kine" align="center" style="margin:20px auto; width:100%;background:#ddd;">
                <tr>
                    <td style="padding:5px;text-align:center;">
                        <h1>Physiotherapy</h1>
						...
						-->
                            ...
                            if($ligneCatPrestaKine=$resultatsPrestaKine->fetch())
                            {
                                ...
                                echo '<option value='.$ligneCatPrestaKine->id_prestation.' onclick="ShowRadio(\'radio\')">'.$ligneCatPrestaKine->nompresta.'</option>';
                            ...
                            <!--
						<option value="autrekine" id="autrekine"><?php echo 'Autre type acte...'; ?></option>
						-->
                        ...
                        <table class='tablesorter' cellpadding=0>
                            ...
                            <tr style="background:#ddd;">
                                ...
                                for($q=0;$q<5;$q++)
                                {
                                    ?>
                                    <td>
                                        <input type='text' name='autreprixprestaKineCCO[]' id='autreprixprestaKineCCO' style='width:70px'/>
                                    </td>
                                    <?php
                                }
                                ?>
		...
		<div style="margin-top:20px; text-align:center;">
			<button type="submit" class="btn-large" name="addupdatebtn" id="addupdatebtn" style="<?php if(isset($_GET['consom']) OR isset($_GET['medoc'])){ echo 'display:inline;';}else{ echo 'display:none;';}?>;">Ajouter</button>
		</div>
	...
	?>
		...
		$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.examenfait=0 AND ml.numero=:num AND ml.id_factureMedLabo=0 AND ml.id_consuLabo=:idconsu ORDER BY ml.id_medlabo');		
		$resultMedLabo->execute(array(
		'num'=>$_GET['num'],
		'idconsu'=>$_GET['idconsu']
		));
		...
		$resultMedConsom=$connexion->prepare('SELECT *FROM med_consom co WHERE co.numero=:num AND co.id_factureMedConsom=0 AND co.id_consuConsom=:idconsu ORDER BY co.id_medconsom');		
		$resultMedConsom->execute(array(
		'num'=>$_GET['num'],
		'idconsu'=>$_GET['idconsu']
		));
...
<input type="hidden" name="idprestaInf[]" style="width:100px;display:none;" id="idprestaInf" value="0"/>
							...
							<td>
								<a href="categoriesbill.php?deleteMedInf=<?php echo $ligneMedInf->id_medinf;?>&manager=<?php echo $_GET['manager'];?>&num=<?php echo $_GET['num'];?>&idconsu=<?php echo $_GET['idconsu'];?>&idmed=<?php echo $_GET['idmed'];?>&dateconsu=<?php echo $_GET['dateconsu'];?>&idtypeconsu=<?php echo $_GET['idtypeconsu'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; width:70%; margin-top:10px;"> 
					...
					<tbody>
						...
						<tr style="text-align:center;">
							...
							<td>
								<a href="categoriesbill.php?deleteMedLabo=<?php echo $ligneMedLabo->id_medlabo;?>&manager=<?php echo $_GET['manager'];?>&num=<?php echo $_GET['num'];?>&idconsu=<?php echo $_GET['idconsu'];?>&idmed=<?php echo $_GET['idmed'];?>&dateconsu=<?php echo $_GET['dateconsu'];?>&idtypeconsu=<?php echo $_GET['idtypeconsu'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<table class="tablesorter tablesorter2" cellspacing="0" style="background:#fff; width:70%; margin-top:10px;"> 
					...
					<tbody>
						...
						<tr style="text-align:center;">
							...
							<td>
								<a href="categoriesbill.php?deleteMedRadio=<?php echo $ligneMedRadio->id_medradio;?>&manager=<?php echo $_GET['manager'];?>&num=<?php echo $_GET['num'];?>&idconsu=<?php echo $_GET['idconsu'];?>&idmed=<?php echo $_GET['idmed'];?>&dateconsu=<?php echo $_GET['dateconsu'];?>&idtypeconsu=<?php echo $_GET['idtypeconsu'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
					...
					<tbody>
						...
						<tr style="text-align:center;">
							...
							<td>
								...
								<input type="text" name="prixprestaConsom[]" style="width:100px;" id="prixprestaConsom"  class="prixprestaConsom"  value="<?php if($lignePresta->prixpresta!=-1){ echo $lignePresta->prixpresta; }else{ echo 0;}?>"/>
						...
						<td>
							<input type="text" id="quantityConsom" name="quantityConsom[]" class="quantityConsom" style="width:30px; text-align:center" value="<?php echo $ligneMedConsom->qteConsom;?>"/>
						...
						<td>
							<a href="categoriesbill.php?deleteMedConsom=<?php echo $ligneMedConsom->id_medconsom;?>&manager=<?php echo $_GET['manager'];?>&num=<?php echo $_GET['num'];?>&idconsu=<?php echo $_GET['idconsu'];?>&idmed=<?php echo $_GET['idmed'];?>&dateconsu=<?php echo $_GET['dateconsu'];?>&idtypeconsu=<?php echo $_GET['idtypeconsu'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
				...
				<tbody>
					...
					<tr style="text-align:center;">
						<td>
							...
							<td>
								<input type="text" id="quantityMedoc" name="quantityMedoc[]" class="quantityMedoc" style="width:30px; text-align:center" value="<?php echo $ligneMedMedoc->qteMedoc;?>"/>
							...
							<td>
								<a href="categoriesbill.php?deleteMedMedoc=<?php echo $ligneMedMedoc->id_medmedoc;?>&manager=<?php echo $_GET['manager'];?>&num=<?php echo $_GET['num'];?>&idconsu=<?php echo $_GET['idconsu'];?>&idmed=<?php echo $_GET['idmed'];?>&dateconsu=<?php echo $_GET['dateconsu'];?>&idtypeconsu=<?php echo $_GET['idtypeconsu'];?>&idassu=<?php echo $_GET['idassu'];?>&idbill=<?php echo $_GET['idbill'];?>&previewprint=ok" class="btn"><i class="fa fa-trash fa-1x fa-fw"></i></a>
		...
		<table style="padding:20px;width:100%">	
			...
			<tr>	
				...
				if(isset($_GET['datefacture']))
				{
					...
					$resultMedConsult=$connexion->prepare('SELECT *FROM med_consult mc WHERE mc.dateconsu=:datefacture AND mc.numero=:num AND mc.id_factureMedConsu=0 ORDER BY mc.id_medconsu');		
					$resultMedConsult->execute(array(
					'num'=>$numPa,
					'datefacture'=>$datefacture	
					));
					...
					$resultMedInf=$connexion->prepare('SELECT *FROM med_inf mi WHERE mi.dateconsu=:datefacture AND mi.soinsfait=1 AND mi.numero=:num AND mi.id_factureMedInf=0 ORDER BY mi.id_medinf');		
					$resultMedInf->execute(array(
					'num'=>$numPa,
					'datefacture'=>$datefacture	
					));
					...
					$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo ml WHERE ml.dateconsu=:datefacture AND ml.examenfait=1 AND ml.numero=:num AND ml.id_factureMedLabo=0 ORDER BY ml.id_medlabo');		
					$resultMedLabo->execute(array(
					'num'=>$numPa,
					'datefacture'=>$datefacture	
					));
					...
					?>
							<tr style="text-align:center;">
								<td>
									...
									$resultPresta->setFetchMode(PDO::FETCH_OBJ);//on veut que le r괵ltat soit rꤵp곡ble sous forme d'objet

									$comptPresta=$resultPresta->rowCount();
									
									if($lignePresta=$resultPresta->fetch())//on recupere la liste des ꭩments
									{
										if($lignePresta->namepresta!='')
										{
											echo $lignePresta->namepresta.'</td>
				...
				<td>
					<form id="formulairetest" method="post" action="billing.php?num=<?php echo $_GET['num'];?>&manager=<?php echo $_GET['manager'];?>&datefacture=<?php if(isset($_POST['datefacture'])){ echo $_POST['datefacture']; }else { if(isset($_GET['datefacture'])) echo $_GET['datefacture'];}?>&facture=ok&search=ok#addBilling">
					...
					?>			
							...
							?>
									...
									<?php 
										...
										$resultcashier->setFetchMode(PDO::FETCH_OBJ);//on veut que le r괵ltat soit rꤵp곡ble sous forme d'objet
									...
									?>
										...
										<td>
											<a href="traitement_billing.php?num=<?php echo $_GET['num'];?>&manager=<?php echo $_GET['manager'];?>&search=ok&datefacture=<?php if(isset($_POST['datefacture'])){ echo $_POST['datefacture']; }else { if(isset($_GET['datefacture'])) echo $_GET['datefacture'];}?>&deleteid_item=<?php echo $ligneTempFacture->id_tempfacture;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" name="deleteTempitem" class="btn"/>Supprimer</a>
										</td>
									</tr> 
								<?php
...
<?php
		...
		/*-------Requ뵥 pour AFFICHER FACTURE-----------*/


		/*-------Requ뵥 pour AFFICHER Type consultation-----------*/
		...
		$resultConsult->setFetchMode(PDO::FETCH_OBJ);//on veut que le r괵ltat soit rꤵp곡ble sous forme d'objet
		...
		/*-------Requ뵥 pour AFFICHER Med_consult-----------*/
		...
		$resultMedConsult->setFetchMode(PDO::FETCH_OBJ);//on veut que le r괵ltat soit rꤵp곡ble sous forme d'objet
		...
		$resultMedInf->setFetchMode(PDO::FETCH_OBJ);//on veut que le r괵ltat soit rꤵp곡ble sous forme d'objet
		...
		/*-------Requ뵥 pour AFFICHER Temp_facture-----------*/
		...
		$resultatsTempFact->setFetchMode(PDO::FETCH_OBJ);//on veut que le r괵ltat soit rꤵp곡ble sous forme d'objet
		...
		if($comptResultTempFact!=0)
		{
			...
			while($ligne=$resultatsTempFact->fetch())//on rꤵp鳥 la liste des ꭩments
			{
...
?>
	...
	<br/>
		...
		<a href="billing.php?num=<?php echo $_GET['num'];?>&manager=<?php echo $_GET['manager'];?>&search=ok" name="printbtn" class="btn-large"/>Report</a>
	...
	{		
		...
		if(more="services" || more==4 || more==3 || more==12 || more==13 || more==21 || more==22 || more==23)
		{
			document.getElementById('addupdatebtn').style.display='inline';
