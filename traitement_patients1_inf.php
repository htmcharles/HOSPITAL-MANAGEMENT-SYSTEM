...
$resultM=$connexion->query('SELECT *FROM consultations c,utilisateurs u, patients p WHERE u.id_u=p.id_u and c.numero=p.numero');
...
if(isset($_GET['sn']))
{
	$result=$connexion->query( 'SELECT * FROM utilisateurs u, patients p WHERE u.id_u=p.id_u AND p.numero LIKE \'%' . safe( $_GET['s'] ) . '%\' LIMIT 0,5');
}


if(isset($_GET['ri']))
{
	$result=$connexion->query( 'SELECT * FROM utilisateurs u, patients p WHERE u.id_u=p.id_u AND p.reference_id LIKE \'%' . safe( $_GET['r'] ) . '%\' LIMIT 0,5');
}
...
?>
			...
			<td id="<?php echo $post->id_u?>">
				...
				<tr>
					<td>
						<a class="<?php if($post->status==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="patients1_inf.php?iduti=<?php echo $post->id_u?>&numPa=<?php echo $post->numero?>&fullname=<?php echo $post->full_name;?>&divPa=ok<?php if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}else{ if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">
							<h4>
							<?php 
								echo $post->full_name;
...
?>
			...
			<td id="<?php echo $post->id_u?>">
				...
				<tr>
					<td>
						<a class="<?php if($post->status==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="patients1_inf.php?iduti=<?php echo $post->id_u?>&numPa=<?php echo $post->reference_id?>&fullname=<?php echo $post->full_name;?>&divPa=ok<?php if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}else{ if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">
