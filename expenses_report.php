...
if(isset($_GET['cash']) OR isset($_SESSION['codeC']))
    {
        if(isset($_GET['coordi'])){
            $result=$connexion->prepare('SELECT *FROM utilisateurs u,coordinateurs co WHERE (co.codecoordi=:operation) AND (u.id_u=co.id_u)');
            //echo "string";
         }else{
            if(isset($_GET['cash'])){
    ...
    ?>
        ...
        if(!isset($_GET['reporthospCash']))
        {
            ...
            <div id="dmacBillReport" style="display:inline">
                ...
                $resultCashierBillReport=$connexion->prepare('SELECT * FROM expenses WHERE codecashier=:doneby '.$dailydateperso.'');
