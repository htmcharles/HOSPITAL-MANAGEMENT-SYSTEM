...
//									var_dump($chartDetails[$chartPositionRef]);
	...
	private static function _readStyle($dorayle, $style) {
		// format code
//		if (isset($style->numFmt)) {
//			if (isset($style->numFmt['formatCode'])) {
//				$dorayle->getNumberFormat()->setFormatCode((string) $style->numFmt['formatCode']);
//			} else {
				$dorayle->getNumberFormat()->setFormatCode($style->numFmt);
//			}
//		}

		// font
		if (isset($style->font)) {
			$dorayle->getFont()->setName((string) $style->font->name["val"]);
			$dorayle->getFont()->setSize((string) $style->font->sz["val"]);
			if (isset($style->font->b)) {
				$dorayle->getFont()->setBold(!isset($style->font->b["val"]) || self::boolean((string) $style->font->b["val"]));
			}
			if (isset($style->font->i)) {
				$dorayle->getFont()->setItalic(!isset($style->font->i["val"]) || self::boolean((string) $style->font->i["val"]));
			}
			if (isset($style->font->strike)) {
				$dorayle->getFont()->setStrikethrough(!isset($style->font->strike["val"]) || self::boolean((string) $style->font->strike["val"]));
			}
			$dorayle->getFont()->getColor()->setARGB(self::_readColor($style->font->color));

			if (isset($style->font->u) && !isset($style->font->u["val"])) {
				$dorayle->getFont()->setUnderline(PHPExcel_Style_Font::UNDERLINE_SINGLE);
			} else if (isset($style->font->u) && isset($style->font->u["val"])) {
				$dorayle->getFont()->setUnderline((string)$style->font->u["val"]);
			}

			if (isset($style->font->vertAlign) && isset($style->font->vertAlign["val"])) {
				$vertAlign = strtolower((string)$style->font->vertAlign["val"]);
				if ($vertAlign == 'superscript') {
					$dorayle->getFont()->setSuperScript(true);
				}
				if ($vertAlign == 'subscript') {
					$dorayle->getFont()->setSubScript(true);
				}
			}
		}
		...
		if (isset($style->fill)) {
			if ($style->fill->gradientFill) {
				...
				if(!empty($gradientFill["type"])) {
					$dorayle->getFill()->setFillType((string) $gradientFill["type"]);
				}
				$dorayle->getFill()->setRotation(floatval($gradientFill["degree"]));
				$gradientFill->registerXPathNamespace("sml", "http://schemas.openxmlformats.org/spreadsheetml/2006/main");
				$dorayle->getFill()->getStartColor()->setARGB(self::_readColor( self::array_item($gradientFill->xpath("sml:stop[@position=0]"))->color) );
				$dorayle->getFill()->getEndColor()->setARGB(self::_readColor( self::array_item($gradientFill->xpath("sml:stop[@position=1]"))->color) );
			} elseif ($style->fill->patternFill) {
				$patternType = (string)$style->fill->patternFill["patternType"] != '' ? (string)$style->fill->patternFill["patternType"] : 'solid';
				$dorayle->getFill()->setFillType($patternType);
				if ($style->fill->patternFill->fgColor) {
					$dorayle->getFill()->getStartColor()->setARGB(self::_readColor($style->fill->patternFill->fgColor,true));
				} else {
					$dorayle->getFill()->getStartColor()->setARGB('FF000000');
				}
				if ($style->fill->patternFill->bgColor) {
					$dorayle->getFill()->getEndColor()->setARGB(self::_readColor($style->fill->patternFill->bgColor,true));
				}
			}
		}
		...
		if (isset($style->border)) {
			...
			if (!$diagonalUp && !$diagonalDown) {
				$dorayle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_NONE);
			} elseif ($diagonalUp && !$diagonalDown) {
				$dorayle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_UP);
			} elseif (!$diagonalUp && $diagonalDown) {
				$dorayle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_DOWN);
			} else {
				$dorayle->getBorders()->setDiagonalDirection(PHPExcel_Style_Borders::DIAGONAL_BOTH);
			}
			self::_readBorder($dorayle->getBorders()->getLeft(), $style->border->left);
			self::_readBorder($dorayle->getBorders()->getRight(), $style->border->right);
			self::_readBorder($dorayle->getBorders()->getTop(), $style->border->top);
			self::_readBorder($dorayle->getBorders()->getBottom(), $style->border->bottom);
			self::_readBorder($dorayle->getBorders()->getDiagonal(), $style->border->diagonal);
		}

		// alignment
		if (isset($style->alignment)) {
			$dorayle->getAlignment()->setHorizontal((string) $style->alignment["horizontal"]);
			$dorayle->getAlignment()->setVertical((string) $style->alignment["vertical"]);
			...
			$dorayle->getAlignment()->setTextRotation(intval($textRotation));
			$dorayle->getAlignment()->setWrapText(self::boolean((string) $style->alignment["wrapText"]));
			$dorayle->getAlignment()->setShrinkToFit(self::boolean((string) $style->alignment["shrinkToFit"]));
			$dorayle->getAlignment()->setIndent( intval((string)$style->alignment["indent"]) > 0 ? intval((string)$style->alignment["indent"]) : 0 );
		}
		...
		if (isset($style->protection)) {
			if (isset($style->protection['locked'])) {
				if (self::boolean((string) $style->protection['locked'])) {
					$dorayle->getProtection()->setLocked(PHPExcel_Style_Protection::PROTECTION_PROTECTED);
				} else {
					$dorayle->getProtection()->setLocked(PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
			}

			if (isset($style->protection['hidden'])) {
				if (self::boolean((string) $style->protection['hidden'])) {
					$dorayle->getProtection()->setHidden(PHPExcel_Style_Protection::PROTECTION_PROTECTED);
				} else {
					$dorayle->getProtection()->setHidden(PHPExcel_Style_Protection::PROTECTION_UNPROTECTED);
				}
			}
		}
		...
		if (isset($style->quotePrefix)) {
			$dorayle->setQuotePrefix($style->quotePrefix);
        }
	}
