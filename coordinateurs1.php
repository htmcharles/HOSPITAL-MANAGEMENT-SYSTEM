...
<head>
	<title>AFFICHE Coorditeur</title>
	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				<li class="">			
					<form method="post" action="coordinateurs1.php?<?php if(isset($_GET['codeCoord'])){ echo 'codeCoord='.$_GET['codeCoord'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
					...
					{
					?>
						<a href="coordinateurs1.php?english=english<?php if(isset($_GET['codeCoord'])){ echo '&codeCoord='.$_GET['codeCoord'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divLabo'])){ echo '&divLabo='.$_GET['divLabo'];}?>" class="btn"><?php echo getString(30);?></a>
					<?php
					}else{
					?>
						<a href="coordinateurs1.php?francais=francais<?php if(isset($_GET['codeCoord'])){ echo '&codeCoord='.$_GET['codeCoord'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divLabo'])){ echo '&divLabo='.$_GET['divLabo'];}?>" class="btn"><?php echo getString(29);?></a>
					<?php
					}					
					?>
						<br/>						
...
?>
			<h2><?php echo getString(179);?>s</h2>
...
<?php 
	if(isset($_GET['divCoord']))
	{
		...
		if($comptidM!=0)
		{
			$resultatsM=$connexion->prepare('SELECT *FROM utilisateurs u, coordinateurs coo WHERE u.id_u=coo.id_u AND coo.id_u=:idCoord AND u.id_u=:idCoord');
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<thead> 
				<tr>
					...
					<th style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>">ACTIONS</th>
					<th style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>">STATUS</th>
			</thead>
			<tbody>
				<?php
				...
				<tr style="text-align:center;<?php if($ligneM->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneM->codecoordi;?></td>
					...
					<td><?php echo $ligneM->dateaffectationcoordi;?></td>
					
					<?php
					if($ligneM->status==1)
					{
					?>
					<td style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>">
						<a href="utilisateurs.php?iduti=<?php echo $ligneM->id_u?>"><input type="image" src="images/icn_edit.png" title="Modifier"/></a>
					</td>

					<td style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>">
						<a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codecoordi;?>&divCoord=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
					...
					<?php
						...
						{
					?>
						<td style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>"></td>
						<td style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>">
							<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codecoordi;?>&divCoord=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
	...
	try
	{
		...
		$resultats=$connexion->query('SELECT *FROM utilisateurs u, coordinateurs coo WHERE u.id_u=coo.id_u ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
		...
		?>
			<tr style="text-align:center;<?php if($ligne->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
				<td><?php echo $ligne->codecoordi;?></td>
				...
				<td><?php echo $ligne->dateaffectationcoordi;?></td>
