...
if(isset($_GET['finishbtn']))
{
	...
	$updateIdFactureHosp=$connexion->prepare('UPDATE patients_hosp ph SET ph.id_factureHosp=:numbill, ph.codecoordiHosp=:codecoordi WHERE ph.id_hosp=:idhosp AND ph.id_factureHosp IS NULL AND ph.numero=:num');
	...
	$updateIdFactureMedSurge=$connexion->prepare('UPDATE med_surge_hosp ms SET ms.id_factureMedSurge=:numbill, ms.codecoordi=:codecoordi WHERE ms.id_hospSurge=:idhosp AND ms.numero=:num AND ms.id_factureMedSurge=""');
	...
	$updateIdFactureMedInf=$connexion->prepare('UPDATE med_inf_hosp mi SET mi.id_factureMedInf=:numbill, mi.codecoordi=:codecoordi WHERE mi.id_hospInf=:idhosp AND mi.numero=:num AND mi.id_factureMedInf=""');
	...
	$updateIdFactureMedLabo=$connexion->prepare('UPDATE med_labo_hosp ml SET ml.id_factureMedLabo=:numbill, ml.codecoordi=:codecoordi WHERE ml.id_hospLabo=:idhosp AND ml.numero=:num AND ml.id_factureMedLabo=""');
	...
	$updateIdFactureMedRadio=$connexion->prepare('UPDATE med_radio_hosp mr SET mr.id_factureMedRadio=:numbill, mr.codecoordi=:codecoordi WHERE mr.id_hospRadio=:idhosp AND mr.numero=:num AND mr.id_factureMedRadio=""');
	...
	$updateIdFactureMedKine=$connexion->prepare('UPDATE med_kine_hosp mk SET mk.id_factureMedKine=:numbill, mk.codecoordi=:codecoordi WHERE mk.id_hospKine=:idhosp AND mk.numero=:num AND mk.id_factureMedKine=""');
	...
	$updateIdFactureMedOrtho=$connexion->prepare('UPDATE med_ortho_hosp mo SET mo.id_factureMedOrtho=:numbill, mo.codecoordi=:codecoordi WHERE mo.id_hospOrtho=:idhosp AND mo.numero=:num AND mo.id_factureMedOrtho=""');
	...
	$updateIdFactureMedConsom=$connexion->prepare('UPDATE med_consom_hosp mco SET mco.id_factureMedConsom=:numbill, mco.codecoordi=:codecoordi WHERE mco.id_hospConsom=:idhosp AND mco.numero=:num AND mco.id_factureMedConsom=""');
	...
	$updateIdFactureMedMedoc=$connexion->prepare('UPDATE med_medoc_hosp mdo SET mdo.id_factureMedMedoc=:numbill, mdo.codecoordi=:codecoordi WHERE mdo.id_hospMedoc=:idhosp AND mdo.numero=:num AND mdo.id_factureMedMedoc=""');
	...
	$updateIdFactureMedConsult=$connexion->prepare('UPDATE med_consult_hosp mc SET mc.id_factureMedConsu=:numbill, mc.codecoordi=:codecoordi WHERE mc.id_hospMed=:idhosp AND mc.numero=:num AND mc.id_factureMedConsu=""');
...
<div class="account-container" style="width:90%">
	...
	<form method="post" action="addupdatepresta_hosp.php?num=<?php echo $_GET['num'];?>&manager=<?php echo $_GET['manager'];?>&idassu=<?php echo $_GET['idassu'];?>&datehosp=<?php echo $_GET['datehosp'];?>&idhosp=<?php echo $_GET['idhosp'];?>&numbill=<?php echo $_GET['numbill'];?>&numroom=<?php echo $_GET['numroom'];?>&numlit=<?php echo $_GET['numlit'];?>&nomassurance=<?php echo $_GET['nomassurance'];?>&billpercent=<?php echo $_GET['billpercent'];?>&datefacture=<?php echo $_GET['datefacture'];?>" onsubmit="return controlFormCategoBillHosp(this)" enctype="multipart/form-data">
		...
		<table align="center">
			...
			$resultMedSurge=$connexion->prepare('SELECT *FROM med_surge_hosp ms WHERE ms.surgefait=0 AND ms.numero=:num AND ms.id_factureMedSurge!="" AND ms.id_hospSurge=:idhosp ORDER BY ms.id_medsurge');
			...
			$resultMedKine=$connexion->prepare('SELECT *FROM med_kine_hosp mk WHERE mk.kinefait=0 AND mk.numero=:num AND mk.id_factureMedKine!="" AND mk.id_hospKine=:idhosp ORDER BY mk.id_medkine');
			...
			$resultMedOrtho=$connexion->prepare('SELECT *FROM med_ortho_hosp mo WHERE mo.orthofait=0 AND mo.numero=:num AND mo.id_factureMedOrtho!="" AND mo.id_hospOrtho=:idhosp ORDER BY mo.id_medortho');
			...
			if($comptHosp!=0 OR $comptMedConsult!=0 OR $comptMedSurge!=0 OR $comptMedInf!=0 OR $comptMedLabo!=0 OR $comptMedRadio!=0 OR $comptMedKine!=0 OR $comptMedOrtho!=0 OR $comptMedConsom!=0 OR $comptMedMedoc!=0)
			{
			?>
					...
					<td style="text-align:left;">
						...
						<table class='tablesorter' cellpadding=0>
							<tbody>
								...
								<tr style='text-align:center;background:#ddd;'>
									<td>Nouveau prix <?php echo $_GET['nomassurance']; ?></td>
									<?php
									for($q=0;$q<5;$q++)
									{
										?>
...
<input type="hidden" name="idprestaInf[]" style="width:100px;display:none;" id="idprestaInf" value="0"/>
	...
	function ShowMore(more)
	{
		...
		if(more="services" || more==4 || more==3 || more==12 || more==13 || more==21 || more==22 || more==14 || more==23)
		{
			document.getElementById('addbtn').style.display='inline';
