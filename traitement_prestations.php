...
if(isset($_GET['name']))
{
	$result=$connexion->query( 'SELECT *FROM prestations p WHERE p.id_categopresta='.$_GET['viewcategopresta'].' AND (p.nompresta LIKE \'%' . safe( $_GET['q'] ) . '%\' OR p.namepresta LIKE \'%' . safe( $_GET['q'] ) . '%\') LIMIT 0,5' );
}

if(isset($_GET['nameconsu']))
{
	$result=$connexion->query( 'SELECT *FROM prestations p WHERE p.id_categopresta='.$_GET['consu'].' AND (p.nompresta LIKE \'%' . safe( $_GET['q'] ) . '%\' OR p.namepresta LIKE \'%' . safe( $_GET['q'] ) . '%\') LIMIT 0,5' );
}
...
<?php
    ...
    ?>
		...
		<td id="<?php echo $post->id_prestation?>">
			...
			<tr>
				<td>
					<a class="searchB" href="prestations.php?iduti=<?php echo $_SESSION['id'];?>&viewcategopresta=<?php echo $_GET['viewcategopresta'];?>&nomcat=<?php echo $_GET['nomcat'];?>&idpresta=<?php echo $post->id_prestation?>&divPresta=ok">
						<h4>
							<?php
    ...
    ?>
		...
		<td id="<?php echo $post->id_prestation?>">
			...
			<tr>
				<td>
					<a class="searchB" href="prestations.php?iduti=<?php echo $_SESSION['id'];?>&consu=<?php echo $_GET['consu'];?>&typeconsu=<?php echo $_GET['typeconsu'];?>&idpresta=<?php echo $post->id_prestation?>&divPrestaConsu=ok">
						<h4>
							<?php
							if($post->namepresta!="" AND $post->nompresta!="")
							{
