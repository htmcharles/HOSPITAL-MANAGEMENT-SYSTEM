...
include("connectLangues.php");
			...
			if(isset($_POST['addQteOrthoBtn'.$idOrtho[$i]]))
			{
				if($qteOrtho[$i]>0)
				{
					...
					if($comptSearchOrtho!=0)				
					{
						...
						{
							$updateQteMedOrtho=$connexion->prepare('UPDATE med_ortho_hosp mo SET mo.datehosp=:datehosp, mo.qteOrtho=:qteOrtho, mo.insupercentOrtho=:insupercentOrtho, mo.prixprestationOrtho=:prixOrtho, mo.id_uI=:id_uI WHERE mo.id_medortho=:idmedortho');
	...
	echo '<script text="text/javascript">document.location.href="categoriesbill_hosp_medecin.php?num='.$_GET['num'].'&idhosp='.$_GET['idhosp'].'&id_uM='.$_GET['id_uM'].'&datehosp='.$_GET['datehosp'].'&idassu='.$_GET['idassu'].'&idbill='.$_GET['idbill'].'&numroom='.$_GET['numroom'].'&numlit='.$_GET['numlit'].'&previewprint=ok&infShow=ok'.$idTable.'"</script>';
