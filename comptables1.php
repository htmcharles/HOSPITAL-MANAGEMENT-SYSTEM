...
<head>
	<title>AFFICHE COMPTABLES</title>
	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				<li class="">			
					<form method="post" action="comptables1.php?<?php if(isset($_GET['codeAcc'])){ echo 'codeAcc='.$_GET['codeAcc'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
					...
					{
					?>
						<a href="comptables1.php?english=english<?php if(isset($_GET['codeAcc'])){ echo '&codeAcc='.$_GET['codeAcc'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divAcc'])){ echo '&divAcc='.$_GET['divAcc'];}?>" class="btn"><?php echo getString(30);?></a>
					<?php
					}else{
					?>
						<a href="comptables1.php?francais=francais<?php if(isset($_GET['codeAcc'])){ echo '&codeAcc='.$_GET['codeAcc'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divAcc'])){ echo '&divAcc='.$_GET['divAcc'];}?>" class="btn"><?php echo getString(29);?></a>
					<?php
					}					
					?>
						<br/>						
...
<ul style="margin-top:20px; margin-bottom:20px; background:none;border:none;">
		...
		<div style="display:none;margin-top:50px;" id="divListe" align="center">
			...
			<h2><?php echo getString(150);?>s</h2>
...
$(document).ready( function() {
  ...
  $('#q').keyup( function(){
	...
	url : 'traitement_comptables1.php?name=ok' , // url du fichier de traitement
	data : 'q='+$(this).val() , // donn? ?nvoyer en  GET ou POST
	...
	success : function(data){ // traitements JS ?aire APRES le retour d'ajax-search.php
		...
		$('#results').html(data); // affichage des r?ltats dans le bloc
	}
      });
    }		
  });
...
$(document).ready( function() {
  ...
  $('#s').keyup( function(){
    ...
    if( $field.val().length > 0 )
    {
      ...
      $.ajax({
  	type : 'GET', // envoi des donn? en GET ou POST
	url : 'traitement_comptables1.php?sn=ok' , // url du fichier de traitement
	...
	if(isset($_GET['divAcc']))
	{
		...
		if($comptidA!=0)
		{
			$resultatsA=$connexion->prepare('SELECT *FROM utilisateurs u,accountants ac WHERE u.id_u=ac.id_u AND ac.id_u=:idAud AND u.id_u=:idAud');
		...
		/*-----------Requ? pour coordinateur-----------*/
		
		if($comptidM!=0)
		{
			$resultatsM=$connexion->prepare('SELECT *FROM utilisateurs u,accountants ac WHERE u.id_u=ac.id_u AND ac.id_u=:idAud AND u.id_u=:idAud');
	...
	?>
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneM->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneM->codeaccount;?></td>
					...
					<td><?php echo $ligneM->dateaffectationaccount;?></td>
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneA->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneA->codeaccount;?></td>
					...
					<td><?php echo $ligneA->dateaffectationaccount;?></td>

					<td>
						<a href="report.php?acc=<?php echo $ligneA->codeaccount;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneA->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
					</td>
		...
		<a href="comptables1.php?iduser=<?php echo $id;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="Show Comptables" class="btn-large">Show Comptables</a>
	<?php
	}
	?>
	...
	try
	{
		...
		// Numero de page (1 par d?ut)
			if( isset($_GET['page']) && is_numeric($_GET['page']) )
				...
				// Numero du 1er enregistrement ?ire
		...
		$resultats=$connexion->query('SELECT *FROM utilisateurs u,accountants ac WHERE u.id_u=ac.id_u ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
		...
		?>
			<tr style="text-align:center;<?php if($ligne->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
				<td><?php echo $ligne->codeaccount;?></td>
				...
				<td><?php echo $ligne->dateaffectationaccount;?></td>
				...
				<td>
					<a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codeaccount;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
				...
				<td></td>
				<td>
					<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codeaccount;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
				...
				?>
						<td></td>
						<td>
							<a href="report.php?acc=<?php echo $ligne->codeaccount;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
						</td>
				<?php
						}
					}
	...
	<tr>
		...
		$nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u,accountants ac WHERE u.id_u=ac.id_u ORDER BY u.nom_u');
