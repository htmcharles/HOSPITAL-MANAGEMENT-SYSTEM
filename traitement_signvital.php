...
?>
			...
			<td id="<?php echo $post->id_u?>">
				...
				<tr>
					<td>
						<a class="<?php if($post->status==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="signvital.php?signvital=ok&iduti=<?php echo $post->id_u?>&numPa=<?php echo $post->numero?>&fullname=<?php echo $post->full_name;?>&divPa=ok<?php if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}else{ if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">
							<h4>
							<?php 
								echo $post->full_name;
					...
					/* 	$fullname=$post->full_name;
						...
						$resultPa=$connexion->prepare("SELECT u.id_u,u.full_name,p.id_u,u.status,p.numero FROM utilisateurs u, patients p WHERE u.id_u=p.id_u AND u.full_name LIKE :fullname");
						$resultPa->execute(array(
						'fullname'=>$fullname
						))or die( print_r($connexion->errorInfo()));
										
						$RowsCount=$resultPa->rowCount();
