...
?>
		...
		<form style="margin-top:25px" method="post" action="traitement_consult.php?num=<?php echo $num;?>&idMed=<?php echo $_SESSION['id'];?>&datetoday=<?php echo $annee;?><?php if(isset($_GET['idconsuNext'])){ echo '&idconsuNext='.$_GET['idconsuNext'];}else{ if(isset($_GET['idconsu'])){ echo '&idconsu='.$_GET['idconsu'];}else{ echo '&idconsult='.$idConsu;}} if(isset($_GET['idtypeconsult'])){ echo '&idtypeconsult='.$_GET['idtypeconsult'];} if(isset($_GET['idassuconsu'])){ echo '&idassuconsu='.$_GET['idassuconsu'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormConsultation(this)" enctype="multipart/form-data">
			...
			*/
						...
						<table style="margin:30px auto auto; width:70%; padding-left:80px;" cellpadding=3>
							...
							<tr>						
								<td style="text-align:left;color:black;">
									<input type="text" name="poids" id="poids"  value="<?php if(isset($_GET['num']) and $comptidI!=0){ echo $poids ;}else{if(isset($_GET['idconsu']) and $comptidM!=0){echo $poids;}else{if(isset($_GET['idconsuNext'])){echo $poids;}else{ echo $poidsPa;}}}?>" <?php if(isset($_GET['num']) and $comptidI!=0){echo "readonly='readonly'";}?> style="width:35px;"/><span style="line-height:37px;">Kg</span>
									
								</td>
								
								<td style="text-align:left;color:black;">
									<input type="text" name="taille" id="taille"  value="<?php if(isset($_GET['num']) and $comptidI!=0){ echo $taille ;}else{if(isset($_GET['idconsu']) and $comptidM!=0){echo $taille;}else{if(isset($_GET['idconsuNext'])){echo $taille;}else{ echo $taillePa;}}}?>" <?php if(isset($_GET['num']) and $comptidI!=0){echo "readonly='readonly'";}?> style="width:35px;"/><span style="line-height:37px;">Cm</span>
									
								</td>
													
								<td style="text-align:left;color:black;">
									<input type="text" name="tempera" id="tempera"  value="<?php if(isset($_GET['num']) and $comptidI!=0){ echo $tempera;}else{if(isset($_GET['idconsu']) and $comptidM!=0){echo $tempera;}else{if(isset($_GET['idconsuNext'])){echo $tempera;}else{ echo $temperaPa;}}}?>" <?php  if(isset($_GET['num']) and $comptidI!=0){ echo 'readonly="readonly"';}else{echo '';}?> style="width:35px;"/><span style="line-height:37px;">°C</span>
								</td>
								
								<td style="text-align:left;color:black;">
									<input type="text" name="tensionart" id="tensionart" value="<?php if(isset($_GET['num']) and $comptidI!=0){ echo $tensionart;}else{if(isset($_GET['idconsu']) and $comptidM!=0){echo $tensionart;}else{ if(isset($_GET['idconsuNext'])){echo $tensionart;}else{ echo $tensionartPa;}}}?>" <?php  if(isset($_GET['num']) and $comptidI!=0){ echo 'readonly="readonly"';}else{echo '';}?> style="width:35px;"/><span style="line-height:37px;">mmHg</span>
									
								</td>
								
								<td style="text-align:left;color:black;">
									<input type="text" name="pouls" id="pouls" value="<?php if(isset($_GET['num']) and $comptidI!=0){ echo $pouls;}else{if(isset($_GET['idconsu']) and $comptidM!=0){echo $pouls;}else{ if(isset($_GET['idconsuNext'])){echo $pouls;}else{ echo $poulsPa;}}}?>" <?php  if(isset($_GET['num']) and $comptidI!=0){ echo 'readonly="readonly"';}else{echo '';}?> style="width:35px;"/><span style="line-height:37px;">/min</span>
						...
						<table class="cons-table" cellpadding=3 style="padding: 5px; margin: 20px auto; background: #ddd none repeat scroll 0px 0px; border: 1px solid rgb(238, 238, 238); border-radius: 4px; width:auto;" align="center" id="motifTable">	
							...
							<tr>
								<td>
									<select style="margin:auto" multiple="multiple" name="motif[]" class="chosen-select" id="motif">
									<?php
									...
									{
									?>
										<option value='<?php echo $ligneMotif->id_motif;?>'>
										<?php
			...
			<table class="cons-info" style="padding: 5px; margin: 20px auto; background: #eee none repeat scroll 0px 0px; border: 1px solid rgb(238, 238, 238); border-radius: 4px; width:45%;"  cellpadding=5 id="diagno">			
				<tr>
				...
				<tr>
					<td>
						<input type="text" style="height:35px; display:inline;" id="areaPrediagno" name="areaPrediagno" placeholder="<?php echo getString(248);?>"/>
		...
		if(isset($_GET['idconsu']))
		{
			...
			$medConsult=$connexion->prepare('SELECT *FROM med_consult mc WHERE mc.id_consuMed=:idConsu AND mc.id_uM=:idM AND mc.numero=:num AND mc.dateconsu=:date ORDER BY mc.id_medconsu');		
			$medConsult->execute(array(
			'idConsu'=>$_GET['idconsu'],
			'num'=>$_GET['num']
			));
			...
			$medInf=$connexion->prepare('SELECT *FROM med_inf mi WHERE mi.id_consuInf=:idConsu AND mi.id_uM=:idM AND mi.numero=:num AND mi.dateconsu=:date ORDER BY mi.id_medinf');		
			$medInf->execute(array(
			'idConsu'=>$idConsu,
			'idM'=>$_SESSION['id'],
			'num'=>$_GET['num']
		...
		?>
			...
			<table class="cons-table" style="padding: 5px; margin: 20px auto; background: #ddd none repeat scroll 0px 0px; border: 1px solid rgb(238, 238, 238); border-radius: 4px; width:85%;"  cellpadding=5>			
				...
				{ */
				?>	
				<tr style="display:none;" id="MedConsom">	
					<td>
		...
		<table style="background: #d8d8d8" class="cons-info" cellpadding=10>
			...
			<tr>
				<td colspan=4>					
					<table class="cons-info" style="padding: 5px; margin: 20px auto; background: #eee none repeat scroll 0px 0px; border: 1px solid rgb(238, 238, 238); border-radius: 4px; width:45%;"  cellpadding=5 id="diagnopost">			
						<tr>
			...
			<tr>
				<td colspan=4>
					<table class="cons-info" cellpadding=5 style="margin:0;">
			...
			<tr>
				...
				<td>
					<table class="cons-table" style="padding: 5px; margin: 20px auto; background: #ddd none repeat scroll 0px 0px; border: 1px solid rgb(238, 238, 238); border-radius: 4px; width:auto;"  cellpadding=3>	
							<tr>	
								<td style="text-align:left">PHYSIOTHERAPY<input type="checkbox" name="physio" id="physio" <?php if($physio==1){ echo "checked=checked";}?> onclick="ShowMotifphysio('physio')"/>
								</td>
							</tr>
					...
					<table class="cons-table" style="padding: 5px; margin: 20px auto; background: #ddd none repeat scroll 0px 0px; border: 1px solid rgb(238, 238, 238); border-radius: 4px; width:auto;"  cellpadding=3 id="tableOrtho">	
						...
						<tr>						
							<td>
								...
								if($ligneCatPrestaOrtho=$resultatsPrestaOrtho->fetch())
								{
									...
									echo '<option value='.$ligneCatPrestaOrtho->id_prestation.' onclick="ShowOthersSoins(\'soins\')">'.$ligneCatPrestaOrtho->nompresta.'</option>';							
						...
						<tr>	
							...
							?>
								...
								<thead> 
									<tr>
										<th style="background: linear-gradient(black, gray);color:white;font-size:150%;padding:5px;">Pas de travail à l'orthopedie</th>
									</tr>
								</thead> 		
								</table>
							<?php
			...
			<tr>				
				...
				<td>
					...
					while($ligneCategoPrestaConsu=$resultatsCategoPrestaConsu->fetch())
					{
						...
						$resultatsPrestaConsu=$connexion->query('SELECT *FROM '.$presta_assu.' p, categopresta_ins cp WHERE cp.id_categopresta=p.id_categopresta AND p.id_categopresta=20 AND p.id_categopresta='.$ligneCategoPrestaConsu->id_categopresta.' ORDER BY p.nompresta');
...
if(isset($_GET['showmore']))
{
	...
	if($comptGetConsult!=0)
	{
		if($ligneGetConsult=$getConsult->fetch())
		{
			...
			if($ligneGetConsult->id_factureConsult!=NULL OR ($nomTypeConsult =="Pas de consultation" OR $nomTypeConsult =="No Consultation"))
			{
?>
	...
	?>
		...
		if($comptgetProfil!=0)
		{
			...
			{
		?>
			<a href="consult.php?num=<?php echo $ligneGetProfil->numero?>&showfiche=ok&idconsu=<?php echo $ligneGetProfil->id_consu;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>#fichepatient" class="btn-large"><span title="View Profile" name="fichebtn"><i class="fa fa-eye fa-lg fa-fw"></i><?php echo getString(100);?></span></a>
...
?>
	...
	*/	
	...
	?>
		...
		<table cellpadding=3 style="background:#fff; border:1px solid #eee; border-radius:4px; margin-bottom:10px; padding: 20px; <?php if($recomm != "") echo 'width:50%;'; else echo 'width:80%;';?>" align="center" id="diagnorecomm">
					...
					try
					{
						while($ligne=$resultats->fetch())//on recupere la liste des éléments
						{
							...
							<tr style="text-align:center;">
								...
								{									
									if($ligne->id_factureConsult !=NULL OR ($nomTypeConsult =="Pas de consultation" OR $nomTypeConsult =="No Consultation"))
									{
								?>
