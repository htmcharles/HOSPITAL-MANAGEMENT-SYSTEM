...
if(isset($_GET['name']))
{
	$result=$connexion->query( 'SELECT *FROM utilisateurs u, auditors a  WHERE u.id_u=a.id_u AND (u.nom_u LIKE \'%' . safe( $_GET['q'] ) . '%\' OR u.prenom_u LIKE \'%' . safe( $_GET['q'] ) . '%\') LIMIT 0,5' );
}

if(isset($_GET['sn']))
{
	$result=$connexion->query( 'SELECT *FROM utilisateurs u,  auditors a  WHERE u.id_u=a.id_u AND a.codeaudit LIKE \'%' . safe( $_GET['s'] ) . '%\' LIMIT 0,5');
}
...
<?php
    ...
    ?>
		...
		<td id="<?php echo $post->id_u?>">
			...
			<tr>
				<td>
					<a class="<?php if($post->status==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="auditeurs1.php?iduti=<?php echo $post->id_u?>&codeAud=<?php echo $post->codeaudit?>&divAud=ok">
						<h4><?php echo $post->nom_u.' '.$post->prenom_u.' ('.$post->codeaudit.')'; ?></h4>
					</a>
				</td>
    ...
    ?>
		...
		<td id="<?php echo $post->id_u?>">
			...
			<tr>
				<td>
					<a class="<?php if($post->status==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="auditeurs1.php?iduti=<?php echo $post->id_u?>&codeAud=<?php echo $post->codeaudit?>&divAud=ok">
						<h4><?php echo $post->codeaudit.' ('.$post->nom_u.' '.$post->prenom_u.')'; ?></h4>
					</a>
				</td>
