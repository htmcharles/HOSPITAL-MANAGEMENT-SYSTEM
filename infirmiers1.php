...
<head>
	<title>AFFICHE INFIRMIERS</title>
	...
	<div class="navbar-inner">
		...
		<div class="container">
			...
			<div class="nav-collapse">
				...
				<li class="">			
					<form method="post" action="infirmiers1.php?<?php if(isset($_GET['codeInf'])){ echo 'codeInf='.$_GET['codeInf'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormPassword(this)">
					...
					{
					?>
						<a href="infirmiers1.php?english=english<?php if(isset($_GET['codeInf'])){ echo '&codeInf='.$_GET['codeInf'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divInf'])){ echo '&divInf='.$_GET['divInf'];}?>" class="btn"><?php echo getString(30);?></a>
					<?php
					}else{
					?>
						<a href="infirmiers1.php?francais=francais<?php if(isset($_GET['codeInf'])){ echo '&codeInf='.$_GET['codeInf'];}if(isset($_GET['iduti'])){ echo '&iduti='.$_GET['iduti'];}if(isset($_GET['divInf'])){ echo '&divInf='.$_GET['divInf'];}?>" class="btn"><?php echo getString(29);?></a>
					<?php
					}					
					?>
						<br/>						
...
$(document).ready( function() {
  ...
  $('#q').keyup( function(){
	...
	url : 'traitement_infirmiers1.php?name=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
...
$(document).ready( function() {
  ...
  $('#s').keyup( function(){
	...
	url : 'traitement_infirmiers1.php?sn=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>' , // url du fichier de traitement
	...
	if(isset($_GET['divInf']))
	{
		...
		if($comptidA!=0)
		{
			$resultatsA=$connexion->prepare('SELECT *FROM utilisateurs u, infirmiers i WHERE u.id_u=i.id_u AND i.id_u=:idInf AND u.id_u=:idInf');
		...
		if($comptidM!=0)
		{
			$resultatsM=$connexion->prepare('SELECT *FROM utilisateurs u, infirmiers i WHERE u.id_u=i.id_u AND i.id_u=:idInf AND u.id_u=:idInf');
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneM->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneM->codeinfirmier;?></td>
					<td><?php echo $ligneM->full_name;?></td>
					<td>
					<?php 
					if($ligneM->inf_hosp==0)
					{
						echo "";
					}elseif($ligneM->inf_hosp==1){
						echo "Hospitalisation uniquement";
					}else{
						if($ligneM->inf_hosp==2)
						echo "Passe partout";
					}
					?>
					</td>
					...
					<td><?php echo $ligneM->dateaffectationinf;?></td>
					...
					<td></td>
					<td>
						<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligneM->id_u?>&code=<?php echo $ligneM->codeinfirmier;?>&divInf=ok<?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
	...
	?>
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneA->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					<td><?php echo $ligneA->codeinfirmier;?></td>
					<td><?php echo $ligneA->full_name;?></td>
					<td>
					<?php 
					if($ligneA->inf_hosp==0)
					{
						echo "";
					}elseif($ligneA->inf_hosp==1){
						echo "Hospitalisation uniquement";
					}else{
						if($ligneA->inf_hosp==2)
						echo "Passe partout";
					}
					?>
					</td>
					...
					<td><?php echo $ligneA->dateaffectationinf;?></td>
					
					<td>
						<a href="report.php?inf=<?php echo $ligneA->id_u;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneA->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
					</td>
		...
		<a href="infirmiers1.php?iduser=<?php echo $id;?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="Show Nurses" class="btn-large">Show All Nurses</a>
	<?php
	}
	?>
	...
	try
	{
		...
		$resultats=$connexion->query('SELECT *FROM utilisateurs u, infirmiers i WHERE u.id_u=i.id_u ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
		...
		?>
			<tr style="text-align:center;<?php if($ligne->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
				<td><?php echo $ligne->codeinfirmier;?></td>
				...
				<td><?php echo $ligne->dateaffectationinf;?></td>
				...
				<td>
					<a href="traitement_utilisateurs.php?idutiDesactif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codeinfirmier;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Activé<input type="image" style="border-radius:100px;" src="images/onBlue.png" name="desactifbtn"/></a>
				...
				<td></td>
				<td>
					<a href="traitement_utilisateurs.php?idutiActif=<?php echo $ligne->id_u?>&code=<?php echo $ligne->codeinfirmier;?><?php if(isset($_GET['page'])){ echo '&page='.$_GET['page'];}else{ echo '&page=1';}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>">Desactivé<input type="image" style="border-radius:100px;" src="images/offBlue.png" name="actifbtn"/></a>
				...
				?>
						<td>
							<a href="report.php?inf=<?php echo $ligne->id_u;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
						</td>
				<?php
						}
					}
	...
	<tr>
		...
		$nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u, infirmiers i WHERE u.id_u=i.id_u ORDER BY u.nom_u');
