...
include("connect.php");
 	...
 	if(isset($_POST['resultatbtn']))
	{
		...
		if (isset($_FILES['autreresult']['name'])) {
		...
		{	
			...
			if (isset($_FILES['autreresult']['name'])) {
				...
				if( $ligneL=$resultIdMedLabo->fetch(PDO::FETCH_OBJ))
				{
					...
					$resultMedLabo=$connexion->prepare("UPDATE med_labo SET examenfait = :examenfait,autreresultats=:autreresult, dateresultats=:dateresult, id_uL=:id_uL WHERE id_medlabo =:idmedLabo");
					$resultMedLabo->execute(array(
						'idmedLabo'=>$idmedlabo[$i],
						'examenfait'=>$examenfait,
						'autreresult'=>$resultats[$i],
					...
					if($phoneno = $GetNumber->fetch(PDO::FETCH_OBJ)){
						...
						$phoneSize = strlen((string) $phonenumber);
						$code = '+25';
					...
					if($countExa == $countDone){
						...
						if($phonenumber!="" AND $phoneSize==10){
