...
if(isset($_GET['deletebill']))
{
	...
	$Update = $connexion->query("UPDATE bills SET status=1 WHERE id_bill=".$id_Bill."");

	echo '<script type="text/javascript"> alert("Bill n° '.$_GET['numbill'].' Of Clinic Deleted Successfuly! ");</script>';
...
if(isset($_GET['deletebillhosp']))
{
	...
	$Update = $connexion->query("UPDATE patients_hosp SET statusBill=1 WHERE id_hosp=".$idhosp."");

	echo '<script type="text/javascript"> alert("Bill n° '.$_GET['numbill'].' Of Hospitalization Deleted Successfuly! ");</script>';
...
<div class="account-container" style="width:95%;margin:auto; text-align:center;">
	...
	?>
		...
		if(!isset($_GET['divAcc']))
		{
			...
			$resultats=$connexion->query("SELECT *FROM bills b WHERE  b.status=0 AND b.numero!='NULL' AND DATE(b.datebill)='".$annee."' ORDER BY b.numbill DESC") ;
