...
if(isset($_GET['q']))
{
	$result=$connexion->query( 'SELECT *FROM patients_hosp ph WHERE ph.statusBill=0 AND ph.id_factureHosp LIKE \'%' . safe( $_GET['q'] ) . '%\'' );
}
    ...
    ?>
		...
		<td id="<?php echo $post->id_factureHosp?>">
			<table cellspacing="10" style="<?php if($post->statusBill==0){?>background:rgba(0,0,0, 0.15); color:#777; <?php ;}else {?>background-color:#fff; color:#a00000;<?php ;}?>; border:1px solid #999; border-radius:3px; padding:2px 9px 2px 9px;">
				...
				<tr>
					<td>
						<a class="<?php if($post->statusBill==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="billsaccount_hosp.php?id_factureHosp=<?php echo $post->id_factureHosp?>&codeAcc=<?php echo $post->codeaccount?>&divBill=ok">
