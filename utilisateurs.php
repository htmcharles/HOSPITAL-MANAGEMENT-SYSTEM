...
include("serialNumber.php");
	...
	/*Empty countresults query Start*/

	$selectCountResult = $connexion->query('SELECT * FROM countresults LIMIT 1');
	$selectCountResult->setFetchMode(PDO::FETCH_OBJ);
	$countResu = $selectCountResult->rowCount();
	if($countResu!=0){

		$ligneCountResult = $selectCountResult->fetch();
		$doneAt = $ligneCountResult->doneAt;
		
		$nowDelete = date('Y').'-'.date('m').'-'.date('d');
		
		if($nowDelete > $doneAt){
			$emptyCountResult = $connexion->prepare('TRUNCATE TABLE `countresults`');
			$emptyCountResult->execute();
		}
	}
	
	/*Empty countresults query End*/
?>
...
<div class="account-container" style="width:70%">
	...
	<form method="post" action="traitement_utilisateurs.php?<?php if(isset($_GET['num'])){ echo 'num='.$_GET['num'];}?><?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" onsubmit="return controlFormUser(this)" enctype="multipart/form-data" class="formlogin">
				...
				<td style="text-align:left">
					...
					?>
						...
						<select name="profil" id="profil" onchange="ShowProfil('Profil')">
							...
							<option value='Coordinateur' id="Coordinateur" style="<?php if($_SESSION['ceo']!=1){echo "display: none;";} ?>"><?php echo getString(179);?></option>

							<option value='Stockepper' id="Stockepper">
							<?php echo getString(281);?>
							</option>
		...
		<fieldset id="fieldStock" <?php if(isset ($_GET['iduti']) and $comptidsto!=0){echo "style='display:inline-block'";}else{echo "style='display:none'";}?>>
		<table>
			<tr>
				<td><label for="codesto"><?php echo getString(7);?></label></td>
				<td>
					<input type="text" name="codesto" id="codesto" value="<?php if(isset ($_GET['iduti']) and $comptidsto!=0){ echo $codesto;}else{ echo showSN('S'); }?>" readonly="readonly"/>
				</td>
			</tr>
		</table>
		</fieldset>
		...
		<div class="add-user">
			...
			?>
				...
				<tr>
					...
					<script src="jQuery.js"></script>
						...
						var provinceChoisi='assurance='+$(this).val();
			...
			<table id="insuranceIDbill" style="<?php if(isset($_GET['iduti']) and $id_assurance != 1){ ?>display:inline-block;<?php }else{ ?>display:none;<?php }?>margin-left:25px">
			...
			<hr>
			<div style="display:none;" id="addPresta">
				<input type="hidden" name="consuType" value="562" id="consuType">
				<input type="hidden" name="id_uM" value="11" id="id_uM">
				<b style="color: green;">Exam :</b> 
				<select style="width:340px;" name="presta" id="presta">
			...
			<tr style="background:red;">
				<td style="text-align:center;">
					<h1 style="font-family:ubuntu;border-bottom:1px solid red;padding-bottom:10px;">Laboratory Exams</h1>
					
					<select style="font-family:ubuntu;margin:auto" multiple="multiple" name="checkprestaLab[]" class="chosen-select" id="checkprestaLab">
						<?php

						$resultatsPrestaExamen=$connexion->query('SELECT *FROM prestations_private p, categopresta_ins cp WHERE p.statupresta!=0 AND cp.id_categopresta=p.id_categopresta AND p.id_categopresta=12 ORDER BY p.nompresta ASC');
						...
						if($ligneCatPrestaExamen=$resultatsPrestaExamen->fetch())
						{
							...
							echo '<option value='.$ligneCatPrestaExamen->nompresta.' onclick="ShowExam(\'exam\')">'.$ligneCatPrestaExamen->nompresta.'</option>';
							
							while($lignePrestaExamen=$resultatsPrestaExamen->fetch())//on recupere la liste des éléments
							{
						?>
								<option value='<?php echo $lignePrestaExamen->nompresta;?>'><?php echo $lignePrestaExamen->nompresta;?></option>
						<?php
							}							
							echo '</optgroup>';
						}
			...
			?>
				...
				<table id="distri" style="display:none; margin-left:157px;">
				<tr>
				...
				<table id="autreAdress" style="display:none; margin-left:157px;">
				...
				<table id="sect" style="display:none; margin-left:155px;">
				<tr>
					<td style="width:50px;"><?php echo getString(43);?></td>
					<td>
						<select style="width:340px;" name="secteur" id="secteur" onchange="ShowAdresse('cell')">
				...
				<script>
					...
					var secteurChoisi='secteur='+$(this).val();
						//alert(secteurChoisi);
						$.ajax({
							url:"provDisSect.php",
							type:"POST",
				...
				<table id="cells" style="display:none; margin-left:155px;">
				<tr>
					<td style="width:50px;"><?php echo getString(293);?></td>
					<td>
						<select style="width:340px;" name="cell" id="cell" onchange="ShowAdresse('village')">
						<option value=""><?php echo getString(295);?></option>
				...
				<table id="villages" style="display:none; margin-left:155px;">
				<tr>
					<td style="width:50px;"><?php echo getString(294);?></td>
					<td>
						<select style="width:340px;" name="village" id="village">
						<option value=""><?php echo getString(296);?></option>
			...
			?>	
				...
				<table id="distri" style="margin-left:89px;">
				<tr>
				...
				<table id="sect" style="margin-left:89px;">
				<tr>
				...
				<table id="cells" style="margin-left:89px;">
				<tr>
					<td style="width:20%;"><?php echo getString(293);?></td>
					<td>
						<select style="width:340px;" name="cell" id="cell" onchange="ShowAdresse('village')">
						<option value=""><?php echo getString(295);?></option>
						<?php

							$resultats=$connexion->prepare('SELECT * FROM cells WHERE id_sector=:id_sector');
							$resultats->execute(array(
							'id_sector'=>$secteur
							));
							while($ligne=$resultats->fetch(PDO::FETCH_OBJ))//on recupere la liste des éléments
							{
						?>
							<option value='<?php echo $ligne->id_cell; ?>'<?php if($cell == $ligne->id_cell){ echo 'selected="selected"';}?>>
							<?php echo $ligne->nomcell;?>
							</option>
						<?php
							}
						?>
				...
				<table id="villages" style="margin-left:89px;">
				<tr>
					<td style="width:20%;"><?php echo getString(294);?></td>
					<td>
						<select style="width:340px;" name="village" id="village">
						<option value=""><?php echo getString(296);?></option>
						<?php

							$resultats=$connexion->prepare('SELECT * FROM villages WHERE id_cell=:id_cell');
							$resultats->execute(array(
							'id_cell'=>$cell
							));
							while($ligne=$resultats->fetch(PDO::FETCH_OBJ))//on recupere la liste des éléments
							{
						?>
							<option value='<?php echo $ligne->id_village; ?>'<?php if($village == $ligne->id_village){ echo 'selected="selected"';}?>>
							<?php echo $ligne->nomvillage;?>
							</option>
						<?php
							}
						?>
			...
			?>
				...
				<tr>
					...
					<td style="text-align:left">
						<input type="text" name="phone" id="phone" onfocus="ShowHelp('phone')" placeholder="<?php echo getString(83);?>" value="<?php if(isset($_GET['iduti'])){ echo $phone;}else{echo '';}?>" required="required"/>*
					</td>
				...
				if(isset($_SESSION['codeC']))
				{
					//echo $_SESSION['ceo'];
				?>
				<tr style="<?php if($_SESSION['ceo']!=1 AND isset($_GET['iduti'])){echo "display: none;";} ?>">
...
<div>
	...
	function ShowAssurance(assure)
	{
		...
		document.getElementById('addPresta').style.display = 'inline-block';
	...
	function ShowAdresse(adresse)
	{
		...
		if( adresse =='cell')
		{
			...
			if( Secteur =='')
			{
				...
				document.getElementById('cells').style.display='inline-block';
			}
		}		
		...
		if( adresse =='village')
		{
			...
			if( Cell =='')
			{
				...
				document.getElementById('villages').style.display='inline-block';
			}
		}
	...
	function ShowProfil(pro)
	{
		...
		if( profil =='Stockepper')
		{
			document.getElementById('fieldStock').style.display='inline-block';
		}else{
			document.getElementById('fieldStock').style.display='none';
		}
