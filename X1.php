...
?>
	...
	?>
		...
		{ */
			...
			if (isset($_GET['sortie'])) {
				$resultMedConsult=$connexion->prepare('SELECT *FROM med_consult_hosp mc WHERE mc.numero=:num AND mc.id_factureMedConsu="" AND mc.id_hospMed=:idhosp GROUP BY mc.id_prestationConsu HAVING COUNT(mc.id_prestationConsu) >= 1 ORDER BY mc.id_medconsu');		
				$resultMedConsult->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedInf=$connexion->prepare('SELECT *FROM med_inf_hosp mi WHERE mi.numero=:num AND mi.id_factureMedInf="" AND mi.id_hospInf=:idhosp GROUP BY mi.id_prestation HAVING COUNT(mi.id_prestation) >= 1 ORDER BY mi.id_medinf');		
				$resultMedInf->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedSurge=$connexion->prepare('SELECT *FROM med_surge_hosp ms WHERE ms.numero=:num AND ms.id_factureMedSurge="" AND ms.id_hospSurge=:idhosp GROUP BY ms.id_prestationSurge HAVING COUNT(ms.id_prestationSurge) >= 1 ORDER BY ms.id_medsurge');		
				$resultMedSurge->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedLabo=$connexion->prepare('SELECT *FROM med_labo_hosp ml WHERE ml.numero=:num AND ml.id_factureMedLabo="" AND ml.id_hospLabo=:idhosp GROUP BY ml.id_prestationExa HAVING COUNT(ml.id_prestationExa) >= 1 ORDER BY ml.id_medlabo');		
				$resultMedLabo->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedRadio=$connexion->prepare('SELECT *FROM med_radio_hosp mr WHERE mr.numero=:num AND mr.id_factureMedRadio="" AND mr.id_hospRadio=:idhosp GROUP BY mr.id_prestationRadio HAVING COUNT(mr.id_prestationRadio) >= 1 ORDER BY mr.id_medradio');		
				$resultMedRadio->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedKine=$connexion->prepare('SELECT *FROM med_kine_hosp mk WHERE mk.numero=:num AND mk.id_factureMedKine="" AND mk.id_hospKine=:idhosp GROUP BY mk.id_prestationKine HAVING COUNT(mk.id_prestationKine) >= 1 ORDER BY mk.id_medkine');		
				$resultMedKine->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedConsom=$connexion->prepare('SELECT *FROM med_consom_hosp mco WHERE mco.numero=:num AND mco.id_factureMedConsom="" AND mco.id_hospConsom=:idhosp GROUP BY mco.id_prestationConsom HAVING COUNT(mco.id_prestationConsom) >= 1 ORDER BY mco.id_medconsom');		
				$resultMedConsom->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedMedoc=$connexion->prepare('SELECT *FROM med_medoc_hosp mdo WHERE mdo.numero=:num AND mdo.id_factureMedMedoc="" AND mdo.id_hospMedoc=:idhosp GROUP BY mdo.id_prestationMedoc HAVING COUNT(mdo.id_prestationMedoc) >= 1 ORDER BY mdo.id_medmedoc');		
				$resultMedMedoc->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
				...
				$resultMedOrtho=$connexion->prepare('SELECT *FROM med_ortho_hosp mo WHERE mo.numero=:num AND mo.id_factureMedOrtho="" AND mo.id_hospOrtho=:idhosp GROUP BY mo.id_prestationOrtho HAVING COUNT(mo.id_prestationOrtho) >= 1 ORDER BY mo.id_medortho');		
				$resultMedOrtho->execute(array(
				'num'=>$_GET['num'],
				'idhosp'=>$_GET['idhosp']
				));
...
<?php
							...
							<tr style="font-weight:bold;">
								...
								<td style="text-align:center;">
									...
									<input type="text" name="prixresto" style="width:100px;" id="prixresto" class="prixresto" value="<?php echo $ligneResto->prixresto;?>" required/><span style="font-size:70%; font-weight:normal;">Rwf</span>
								</td>


								<td style="text-align:center;">

									<input type="text" name="prixrestoCCO" style="width:100px;" id="prixrestoCCO" class="prixrestoCCO" value="<?php echo $ligneResto->prixrestoCCO;?>" required/><span style="font-size:70%; font-weight:normal;">Rwf</span>
								</td>
								<td style="text-align:center;">
								
									<input type="text" name="percentResto" class="percentHosp" id="percentResto" style="width:30px; text-align:center" value="<?php echo 100;?>"/> %
								</td>
...
<div>
	<footer style="bottom: 0px; width: 100%; text-align: center; background: #fff; border-top: 1px solid #eee; padding: 10px 0px; vertical-align: middle;">
		<p style="margin:0"><span style="color:#a00000">Clinic Plus®</span> is a product of <span style="font-style:bold;">CodeBlock</span>. ©2015-<?php echo date('Y');?> All rights reserved.</p>
	</footer>
</div>
