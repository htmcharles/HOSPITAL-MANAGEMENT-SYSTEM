...
$(document).ready( function() {
  ...
  $('#s').keyup( function(){
	...
	if(isset($_GET['divCash']))
	{
		...
		if($comptidA!=0)
		{
			$resultatsA=$connexion->prepare('SELECT *FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND c.id_u=:idCash AND u.id_u=:idCash AND c.codecashier LIKE \'CCOC%\' ');
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneM->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					...
					<td>
						<a href="report.php?cash=<?php echo $ligneM->codecashier;?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneM->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
					</td>
		...
		<table class="tablesorter" cellspacing="0" style="background-color:#FFF;width:100%;margin-bottom:20px;">
			...
			<tbody>
				...
				<tr style="text-align:center;<?php if($ligneA->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
					...
					<td>
						<a href="report.php?cash=<?php echo $ligneA->codecashier;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligneA->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
					</td>
	...
	try
	{
		...
		$resultats=$connexion->query('SELECT *FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND (c.codecashier LIKE \'CCOC%\' OR c.codecashier LIKE \'PER%\') ORDER BY u.nom_u LIMIT '.$limit_start.', '.$pagination.'') or die( print_r($connexion->errorInfo()));
		...
		?>
			<tr style="text-align:center;<?php if($ligne->status==0){?>background:rgb(253,168,170)<?php ;}?>"> 
				...
				<td>
					<a href="report.php?cash=<?php echo $ligne->codecashier;?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
				</td>
				...
				?>
					...
					<td>
						<a href="report.php?cash=<?php echo $ligne->codecashier;?>&audit=<?php echo $_SESSION['id'];?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="<?php if($ligne->status==0){ echo "display:none";}?>" class="btn"><?php echo getString(94);?></a>
					</td>
	...
	<tr>
		...
		$nb_total=$connexion->query('SELECT COUNT(*) AS nb_total FROM utilisateurs u, cashiers c WHERE u.id_u=c.id_u AND (c.codecashier LIKE \'CCOC%\' OR c.codecashier LIKE \'PER%\') ORDER BY u.nom_u ');
