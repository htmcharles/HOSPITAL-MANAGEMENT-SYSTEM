...
$result=$connexion->query( 'SELECT *FROM bills b
						  WHERE b.status=1 AND b.numbill LIKE \'%' . safe( $_GET['b'] ) . '%\'
                          LIMIT 0,5' );
    ...
    ?>
		...
		<td id="<?php echo $post->id_bill?>">
			<table cellspacing="10" style="<?php if($post->status==0){?>background:rgba(0,0,0, 0.15); color:#777; <?php ;}else {?>background-color:#fff; color:#a00000;<?php ;}?>; border:1px solid #999; border-radius:3px; padding:2px 9px 2px 9px;">
				...
				<tr>
					<td>
						<a class="<?php if($post->status==0){?>searchA<?php ;}else {?>searchB<?php ;}?>" href="billsaccount.php?idbill=<?php echo $post->id_bill?>&codeAcc=<?php echo $post->codeaccount?>&divAcc2=ok">
