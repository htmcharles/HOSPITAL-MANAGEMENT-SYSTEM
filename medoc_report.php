...
<head>
	<title>Medicaments Reports</title>
...
?>
	<div style="text-align:center;margin-top:20px;">
		...
		<a href="medoc_report.php?medocreport=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn-large" name="savebtn" style="font-size:20px;height:40px; padding:10px 40px;">
			<?php echo 'Reports';?>
		</a>
...
<div class="account-container" style="width:90%; text-align:center;">
	...
	?>
		<ul>
			<?php
			if(isset($_GET['num']) OR isset($_GET['gnlreport']) OR isset($_GET['cash']) OR isset($_GET['med']) OR isset($_GET['inf']) OR isset($_GET['lab']) OR isset($_GET['medocreport']) OR isset($_GET['rec']))
			{
			?>
	...
	?>
		...
		?>
			...
			$resultLaboPa=$connexion->prepare('SELECT *FROM med_medoc mdo WHERE ml.numero=:num AND ml.examenfait=1 '.$dailydateperso.' ORDER BY ml.dateconsu DESC');		
			...
			<table style="width:100%;">
				<tr>
					<td style="text-align:center; width:33.333%;">
						...
						<a href="dmacreport_medoc.php?num=<?php echo $_GET['num'];?>&dailydateperso=<?php echo $dailydateperso;?>&paVisit=<?php echo $paVisit;?>&divPersoMedicReport=ok" style="text-align:center" id="dmacmedicalpersopreview">
			...
			<table class="tablesorter tablesorter3" cellspacing="0" style="background:#fff;"> 
				...
				$compteur=1;
					...
					{
						$idassuMedoc=$ligneLaboPa->id_assuMedoc;
				...
				?>						
						<tr>
							...
							<td style="text-align:center;">
								...
								'prestaId'=>$ligneLaboPa->id_prestationMedoc
								));
							...
							{
								$resultMoreMedLabo=$connexion->prepare('SELECT *FROM more_med_labo mml WHERE mml.numero=:num AND mml.id_medmedoc=:idmedLab ORDER BY mml.id_moremedlabo');		
								$resultMoreMedLabo->execute(array(
								'num'=>$_GET['num'],
								'idmedLab'=>$ligneLaboPa->id_medmedoc
								));
								...
								<tr>
									...
									<?php
										$resultPresta=$connexion->prepare('SELECT *FROM '.$presta_assuMedoc.' p WHERE p.id_prestation=:prestaId');		
										$resultPresta->execute(array(
										'prestaId'=>$ligneMoreMedLabo->id_prestationMedoc
										));
							...
							{
								$resultSpermoMedLabo=$connexion->prepare('SELECT *FROM spermo_med_labo sml WHERE sml.numero=:num AND sml.id_medmedoc=:idmedLab ORDER BY sml.id_spermomedlabo');		
	...
	?>
		<table style="margin:auto;">
			<tr>
				<td style="font-size:18px; width:33.333%;display:inline" id="individualstring">
					<b><h2>MEDICAMENT GENERAL REPORT</h2></b>
				</td>
		...
		<div id="selectdatePersoMedicReport">
			...
			<form action="medoc_report.php?report=ok&dmac=ok&selectGnlMedocReport=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>&medocreport=ok" method="post" style="margin:auto;padding:5px;width:90%;" onsubmit="return controlFormCustom(this)" >
				...
				<table id="dmac" style="margin:auto auto 20px">
					...
					<tr id="dmacligne" style="visibility:visible">
						...
						<td id="dailymedicPerso" style="display:none;">Select Medicament/Date
							
							<select style='margin:auto' class='chosen-select' name='dailyIdmedocPerso' id='dailyIdmedocPerso' onchange="OtherDailyMedoc('dailyMedoc')">
								<?php
							
								$req=$connexion->query('SELECT *FROM prestations_private p WHERE p.id_categopresta=22 ORDER BY p.nompresta ASC');
								...
								?>
									<option value='0'><?php echo 'Autre Medicament...';?></option>
							</select>
							
							<input type="text" id="dailyMedocPerso" name="dailyMedocPerso" value="" placeholder="Enter the medicament..." style="display:none"/>
						...
						<td id="monthlymedicPerso" style="display:none">Select Medicament/Month
						
							<select style='margin:auto' class='chosen-select' name='monthlyIdmedocPerso' id='monthlyIdmedocPerso' onchange="OtherDailyMedoc('monthlyMedoc')">
								<?php
							...
							<input type="text" id="monthlyMedocPerso" name="monthlyMedocPerso" value="" placeholder="Enter the Medicament..." style="display:none"/>
						...
						<td id="annualymedicPerso" style="display:none">Select Medicament/Year
						
							<select style='margin:auto' class='chosen-select' name='annualyIdmedocPerso' id='annualyIdmedocPerso' onchange="OtherDailyMedoc('annualyMedoc')">
								<?php
							...
							<input type="text" id="annualyMedocPerso" name="annualyMedocPerso" value="" placeholder="Enter the Medicament..." style="display:none"/>
						...
						<td id="custommedicPerso" style="display:none">
							...
							<table>
								<tr>
									<td>Select Medicament/...</td>
									<td style="width:250px" >
										
										<select style='margin:auto' class='chosen-select' name='customIdmedocPerso' id='customIdmedocPerso' onchange="OtherDailyMedoc('customMedoc')">
											<?php
										...
										<input type="text" id="customMedocPerso" name="customMedocPerso" value="" placeholder="Enter the Medicament..." style="display:none"/>
		...
		if(isset($_GET['dmac']) OR isset($_GET['selectGnlMedocReport']))
		{
			$dailydateperso = " AND ml.dateresultats != '0000-00-00'";
			$paVisit="gnlPersoMedic";
			...
			if(isset($_POST['searchdailyPerso']))
			{
				if(isset($_POST['dailydatePerso']))
				{
					$getMedMedoc=$connexion->query('SELECT *FROM med_medoc mdo WHERE mdo.dateconsu=\''.$_POST['dailydatePerso'].'\'');
					
					$getMedMedoc->setFetchMode(PDO::FETCH_OBJ);
					
					$arrayMedMedoc=array();
					
					while($ligneMedMedoc=$getMedMedoc->fetch())
					{
						$idMedMedoc=$ligneMedMedoc->id_medmedoc;
						$idprestaMedMedoc=$ligneMedMedoc->id_prestationMedoc;
						$idassuMedMedoc=$ligneMedMedoc->id_assuMedoc;
							
						$arrayMedMedoc[$idMedMedoc]=(array)$ligneMedMedoc;	
					...
					while($ligneNomAssu=$getAssuConsu->fetch())
					{
						...
						$getNomPresta=$connexion->prepare('SELECT *FROM '.$presta_assuMedoc.' p WHERE p.nompresta=:nompresta');
						$getNomPresta->execute(array(
						'nompresta'=>$_POST['dailyIdmedocPerso']
						));
					...
					$arrayMedocReport=array_filter($arrayMedMedoc, function($v) use($arrayIdAssuIdPresta)
					{
						foreach($arrayIdAssuIdPresta as $idAssu => $idPresta)
						{
							if($v['id_assuMedoc']==$idAssu AND $v['id_prestationMedoc']==$idPresta)
							{
								return true;
							}
						}
					...
					{
						$stringIdMedMedoc= $stringIdMedMedoc.''.$medmedoc['id_medmedoc'].',';
						$stringPa= $stringPa.'\''.$medmedoc['numero'].'\',';
						$stringDr= $stringDr.''.$medmedoc['id_uM'].',';
						
						if($medmedoc['codecashier'] !=NULL)
						{
							$stringCaisse= $stringCaisse.'\''.$medmedoc['codecashier'].'\',';
						}
						
						if($medmedoc['id_uInfMedoc'] !=NULL)
						{
							$stringInf= $stringInf.''.$medmedoc['id_uInfMedoc'].',';
						}
					}
					
					$stringIdMedMedoc = rtrim($stringIdMedMedoc, ',').')';
					if($stringPa=='(')
					{
					...
					foreach($arrayMedocReport as $key => $medmedoc)
					{
						$numero= $medmedoc['numero'];
						$arrayMedocReport[$key]['numero'] = $arrayListPatient[$numero];
					}
					
					foreach($arrayMedocReport as $key => $medmedoc)
					{
						$idDr= $medmedoc['id_uM'];
						$arrayMedocReport[$key]['id_uM'] = $arrayListDr[$idDr];
					}
					...
					foreach($arrayMedocReport as $key => $medmedoc)
					{
						if($medmedoc['codecashier']!=NULL)
						{
							$idCaisse= $medmedoc['codecashier'];
							$arrayMedocReport[$key]['codecashier'] = $arrayListCaisse[$idCaisse];
						}
					}
					
					foreach($arrayMedocReport as $key => $medmedoc)
					{
						if($medmedoc['id_uInfMedoc']!=0)
						{
							$idInf= $medmedoc['id_uInfMedoc'];
							$arrayMedocReport[$key]['id_uInfMedoc'] = $arrayListInf[$idInf];
						}
					}
					...
					// $dailydateperso = ' AND ml.dateresultats=\''.$_POST['dailydatePerso'].'\' AND (ml.id_prestationMedoc='.$id_exa.' OR ml.autreExamen LIKE \''.$nomExa.'\')';
					
					$paVisit="dailyPersoMedic";
					
					$stringResult="Daily results : ".$_POST['dailydatePerso']." ( ".$_POST['dailyIdmedocPerso']." )";
			...
			if(isset($_POST['searchmonthlyPerso']))
			{
				...
				{					
					...
					$daysmonth= cal_days_in_month(CAL_GREGORIAN,$ukwezi,$umwaka);
					$getMedMedoc=$connexion->query('SELECT *FROM med_medoc mdo WHERE mdo.dateconsu>=\''.$umwaka.'-'.$ukwezi.'-01\' AND mdo.dateconsu<=\''.$umwaka.'-'.$ukwezi.'-'.$daysmonth.'\'');
					...
					while($ligneNomAssu=$getAssuConsu->fetch())
					{
						...
						$getNomPresta=$connexion->prepare('SELECT *FROM '.$presta_assuMedoc.' p WHERE p.nompresta=:nompresta');
						$getNomPresta->execute(array(
						'nompresta'=>$_POST['monthlyIdmedocPerso']
						));
					...
					$stringResult="Monthly results : ".$_POST['monthlydatePerso']."-".$_POST['monthlydatePersoYear']." ( ".$_POST['monthlyIdmedocPerso']." )";
			...
			if(isset($_POST['searchannualyPerso']))
			{
				if(isset($_POST['annualydatePerso']))
				{
					...
					$getMedMedoc=$connexion->query('SELECT *FROM med_medoc mdo WHERE mdo.dateconsu>=\''.$year.'-01-01\' AND mdo.dateconsu<=\''.$year.'-12-31\'');
					...
					while($ligneNomAssu=$getAssuConsu->fetch())
					{
						...
						$getNomPresta=$connexion->prepare('SELECT *FROM '.$presta_assuMedoc.' p WHERE p.nompresta=:nompresta');
						$getNomPresta->execute(array(
						'nompresta'=>$_POST['annualyIdmedocPerso']
						));
					...
					// $dailydateperso = 'AND ml.dateresultats>=\''.$year.'-01-01\' AND ml.dateresultats<=\''.$year.'-12-31\' AND (ml.id_prestationMedoc='.$id_exa.' OR ml.autreExamen LIKE \''.$nomExa.'\')';
					
					$paVisit="annualyPersoMedic";
					
					$stringResult="Annualy results : ".$_POST['annualydatePerso']." ( ".$_POST['annualyIdmedocPerso']." )";
			...
			if(isset($_POST['searchcustomPerso']))
			{
				if(isset($_POST['customdatedebutPerso']) AND isset($_POST['customdatefinPerso']))
				{
					...
					$getMedMedoc=$connexion->query('SELECT *FROM med_medoc mdo WHERE mdo.dateconsu>=\''.$debut.'\' AND mdo.dateconsu<=\''.$fin.'\'');
					...
					while($ligneNomAssu=$getAssuConsu->fetch())
					{
						...
						$getNomPresta=$connexion->prepare('SELECT *FROM '.$presta_assuMedoc.' p WHERE p.nompresta=:nompresta');
						$getNomPresta->execute(array(
						'nompresta'=>$_POST['customIdmedocPerso']
						));
					...
					// $dailydateperso = 'AND ml.dateresultats>=\''.$debut.'\' AND ml.dateresultats<=\''.$fin.'\' AND (ml.id_prestationMedoc='.$id_exa.' OR ml.autreExamen LIKE \''.$nomExa.'\')';
					
					$paVisit="customPersoMedic";
					
					$stringResult="Customer results : [ ".$_POST['customdatedebutPerso']."/".$_POST['customdatefinPerso']." ] ( ".$_POST['customIdmedocPerso']." )";
		...
		?>
			...
			<table style="width:100%;">
				<tr>
					<td style="text-align:center; width:33.333%;">
						...
						<form method="post" action="dmacreport_medoc.php?dailydateperso=<?php echo $dailydateperso;?>&paVisit=<?php echo $paVisit;?>&divPersoMedicReport=ok&medocreport=ok" style="text-align:center" id="dmacmedicalpersopreview">
				...
				<table class="tablesorter tablesorter3" cellspacing="0" style="background:#fff;width:100%;"> 
					<thead> 
						<tr>
							...
							<th style="text-align:center;"><?php echo 'Medicaments';?></th>
					...
					?>						
							<tr>
								...
								<?php
									echo $arrayMedocReport[$key]['numero']['full_name'];
								?>
								</td>
								...
								{
									echo date('d-M-Y',strtotime($arrayMedocReport[$key]['dateconsu']));
								}
								?>
								</td>
								...
								<td style="text-align:center;">
									...
									if($lignePresta=$resultPresta->fetch())
									{
										...
										$presta=$arrayMedocReport[$key]['autreMedoc'];
									}
										echo $presta;
									?>
								...
								{
									echo $arrayMedocReport[$key]['codecashier']['full_name'];
								}
								?>
								</td>	
								...
								{
									echo $arrayMedocReport[$key]['id_uM']['full_name'];
								}
								?>						
								</td>
								...
								{
									echo $arrayMedocReport[$key]['id_uInfMedoc']['full_name'];
								}
								?>						
								</td>
			...
			?>
				<table class="tablesorter tablesorter4" cellspacing="0" style="background:#fff; width:100%; margin-top:10px;"> 
					...
					<thead>
						<tr>
							<th style="width:12%;text-align:center">No Medicaments Report for this search</th>
						</tr> 
					</thead> 
				</table> 
...
function OtherDailyMedoc(fld){

	if(fld="dailyMedoc")
	{
		var idExa = document.getElementById('dailyIdmedocPerso').value;
		
		if(idExa==0)
		{
			document.getElementById('dailyMedocPerso').style.display='inline';
		}else{
			document.getElementById('dailyMedocPerso').style.display='none';
		}
	}
	
	if(fld="monthlyMedoc")
	{
		var idExa = document.getElementById('monthlyIdmedocPerso').value;
		
		if(idExa==0)
		{
			document.getElementById('monthlyMedocPerso').style.display='inline';
		}else{
			document.getElementById('monthlyMedocPerso').style.display='none';
		}
	}
	
	if(fld="annualyMedoc")
	{
		var idExa = document.getElementById('annualyIdmedocPerso').value;
		
		if(idExa==0)
		{
			document.getElementById('annualyMedocPerso').style.display='inline';
		}else{
			document.getElementById('annualyMedocPerso').style.display='none';
		}
	}
	
	if(fld="customMedoc")
	{
		var idExa = document.getElementById('customIdmedocPerso').value;
		
		if(idExa==0)
		{
			document.getElementById('customMedocPerso').style.display='inline';
		}else{
			document.getElementById('customMedocPerso').style.display='none';
		}
	}
}
</script>
...
?>
	...
	<script type="text/javascript">
		...
		$('#dailyIdmedocPerso').chosen({width:"350px", search_contains: true, inherit_select_classes: true});
		$('#monthlyIdmedocPerso').chosen({width:"350px", search_contains: true, inherit_select_classes: true});
		$('#annualyIdmedocPerso').chosen({width:"350px", search_contains: true, inherit_select_classes: true});
		$('#customIdmedocPerso').chosen({width:"350px", search_contains: true, inherit_select_classes: true});
	</script>
