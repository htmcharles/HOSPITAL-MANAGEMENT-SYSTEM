...
include("connect.php");
	...
	if(isset($_POST['save_expenses_btn'])){
		...
		if(!empty($expensesname) && !empty($Amount)){
			$saveexpenses=$connexion->prepare("INSERT INTO `expenses`(`expensename`, `Motif`,`amount`,`datebill`, `codecashier`) VALUES (:expensesname,:Motif,:Amount,:datebill,:doneby)");
...
<?php
	if(isset($_SESSION['codeC'])){?>
		...
		<!-- <a href="capitaux.php?&codecashier=<?php if(isset($_SESSION['codeCash'])){echo $_SESSION['codeCash'];} ?>&codeCoord=<?php if(isset($_SESSION['codeC'])){echo  $_SESSION['codeC'];}?>&capitaux=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn-large-inversed" name="billListbtn" style="font-size:20px;height:40px; padding:10px 40px;margin-left: 10px;">
			<?php echo getString(278);?>
		</a> -->
...
<div class="account-container" style="width:90%; text-align:center;">
	...
	<ul>
			<li style="width:50%;">
				<a href="expenses.php?iduser=<?php echo $_SESSION['id'];?>&codeCoord=<?php if(isset($_SESSION['codeC'])){echo $_SESSION['codeC'];} ?>&codecashier=<?php if(isset($_SESSION['codeCash'])){ echo $_SESSION['codeCash'];} ?>&expensesList=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" style="margin-right:5px;" data-title="List Of Expenses"><b><i class="fa fa-list fa-lg"></i> List Of Expenses</b>
				</a>
			</li>
...
<?php
	    ...
	    $selectExpe=$connexion->prepare("SELECT * FROM expenses WHERE codecashier=:doneby AND datebill=:datebill");
		$selectExpe->execute(['datebill'=>$now,'doneby'=>$doneby]);
...
<div class="expensesList">
	<div class="container-fluid">
		<div style="width: 50%;position: relative;left: 47%;font-size: 20px;padding-bottom: 20px;font-weight: bold;">Expenses List</div>
		<div style="width: 50%;position: relative;left: 40%;font-size: 20px;padding-bottom: 20px;font-weight: bold;">
      ...
      <!-- 		<a href="expenses_report.php?cash=<?php if(isset($_GET['codeCash'])){echo $_SESSION['codeCash'];}else{echo $_SESSION['codeCash'];}?>&coordi=<?php echo $_SESSION['id'];?><?php if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}else{ if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}}?>&report=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn-large-inversed" style="margin-left: 10px;">
		...
		<?php if($expecount!=0 AND isset($_SESSION['codeCash'])):?>
			...
			<tr>
			    ...
			    <td><?php 
					...
					$selectUsername->execute(['numero'=>$ligneexpe->codecashier]);
			     ...
			     ?></td>
			    <td><a href="expenses.php?cash=<?php if(isset($_SESSION['codeCash'])){ echo $_SESSION['codeCash'];}?>&codecoordi=<?php if(isset($_SESSION['codeC'])){ echo $_SESSION['codeC'];}?>&expeid=<?php echo $ligneexpe->expeid;?>&edit=ok" class="btn-large">Edit</a>

			    	<a href="expenses.php?cash=<?php if(isset($_SESSION['codeCash'])){ echo $_SESSION['codeCash'];}?>&codecoordi=<?php if(isset($_SESSION['codeC'])){ echo $_SESSION['codeC'];}?><?php if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}else{ if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}}?>&expeid=<?php echo $ligneexpe->expeid;?>&delete=ok" class="btn-large-inversed">  <i class="fa fa-trash fa-fw"></i> Delete</a>
			    </td>
			    <?php 
		...
		<?php if($expecount!=0 AND isset($_SESSION['codeC'])):?>
			...
			<tr>
			    ...
			    <td><?php echo $ligneexpe->amount.' ';?>Rwf</td>
			    <td><?php 
			    	$selectUsername=$connexion->prepare("SELECT * FROM utilisateurs u,coordinateurs c WHERE u.id_u=c.id_u  AND c.codecoordi=:numero");
					$selectUsername->execute(['numero'=>$ligneexpe->codecashier]);
					$selectUsername->setFetchMode(PDO::FETCH_OBJ);
					$ligneselect=$selectUsername->fetch();
					$expecount=$selectUsername->rowCount();
...
?>
	...
	<div class="edit">
		...
		<div style="width: 50%;position: relative;left: 45%;font-size: 20px;padding-bottom: 20px;font-weight: bold;font-family: verdana,arial,sans-serif;">
			...
			<a href="expenses_report.php?<?php if(isset($_SESSION['codeCash'])){ echo 'cash='.$_SESSION['codeCash'];}?><?php if(isset($_SESSION['codeC'])){ echo '&codecoordi='.$_SESSION['codeC'];}?><?php if(isset($_GET['receptioniste'])){ echo '&receptioniste='.$_GET['receptioniste'];}else{ if(isset($_GET['caissier'])){ echo '&caissier='.$_GET['caissier'];}}?>&expenses=ok<?php if(isset($_GET['english'])){ echo '&english='.$_GET['english'];}else{ if(isset($_GET['francais'])){ echo '&francais='.$_GET['francais'];}}?>" class="btn-large-inversed" style="margin-left: 20px;">
				<?php echo 'Expenses Report';?>
			</a>
...
if (isset($_GET['delete'])) {
	...
	if($deleteexpenses){
	echo '<script text="text/javascript">document.location.href="expenses.php?expensesList=ok&cash=<?php echo $_SESSION["codeCash"]; ?>"</script>';	
}
}
